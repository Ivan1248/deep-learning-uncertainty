...
\zahvala{zahvala} %Zahvaljujem prof. dr. sc. Siniši Šegviću na tome što se puno trudi pomagati.}
...
\citet{Kendall:2017:WUNBDLCV} su koristili aproksimaciju bayesovske neuronske mreže dropoutom, što je opisano u pododjeljku~\ref{subsec:bnm-dropout}, i empirijski su pokazali da se uz modeliranje aleatorne nesigurnosti uz gubitke u jednadžbama~\eqref{eq:aleatorni-gubitak-regresija}~i~\eqref{eq:aleatorni-gubitak-klasifikacija} može dobiti malo poboljšanje predikcija na zadacima regresije dubine i semantičke segmentacije.
...
Kako bi razlikovali aleatornu i epistemičku nesigurnosti kod semantičke segmentacije, \citet{Kendall:2017:WUNBDLCV} aleatornu nesigurnost modeliraju pomoću predikcije varijanci logita svakog piksela kao što je opisano u pododjeljku~\ref{subsec:eksplicitno-modeliranje-aleatorne}, tj. gubitak za svaki piksel je negativni logaritam vjerojatnosti stvarne klase, ali vjerojatnosti se računaju kao očekivanje izlaza softmaksa po razdiobi logita, kao što je opisano jednadžbom~\eqref{eq:kendall-gubitak-klasifikacija}. Očekivanje se procjenjuje \textit{Monte Carlo} aprosksimacijom. Za semantičku segmentaciju se ukupan gubitak jedne slike računa kao srednji gubitak po pikselima. Kao mjeru epistemičke nesigurnosti, \citet{Kendall:2017:WUNBDLCV} za svaki piksel koriste entropiju njegove izlazne razdiobe dobivene uz \textit{MC-dropout}.

Ovaj pristup nije do kraja ostvaren i umjesto njega je korišten \textit{MC-dropout} uz procjenu epistemičke nesigurnosti međusobnom informacijom \citep{Smith:2018:UMUAED} kao što je opisano u pododjeljku~\ref{subsec:mi-epistemicka}.

\subsubsection{Modeli i skupovi podataka}

Za ovaj rad su ispitane neke od tih ideja na zadatku semantičke segmentacije. Umjesto mreže koju su koristili \citet{Kendall:2017:WUNBDLCV}, korištena je mreža LadderDensenet-121, za koju je potrebno manje memorije. Ona je opisana u \citet{Kreso:2017:LSDFSSLNI}. Korištena je vlastita implementacija te mreže koja ne postiže jednako dobru performansu kao originalna. Na validacijskom skupu Cityscapesa \citep{Cordts:2016:Cityscapes} otprilike postiže srednji omjer presjeka i unije $\mIoU=0.6721\pm0.007$ i točnost piksela $A=0.9459\pm0.0006$ (sredina i standardna devijacija skupa od $5$ mjerenja), što je značajno manje od $\mIoU=0.7282$ i $A=0.9506$ kod originalne implementacije. Na skupu za testiranje CamVida \citep{Brostow:2008:CamVid} ista mreža uz isti postupak učenja postiže otprilike $\mIoU=0.6774\pm0.0031$ i $A=0.9176\pm0.0025$. Navedeni razultati evaluacije se odnose na mrežu kod koje se ne koristi \textit{dropout}. Pokazalo se da dodavanjem dropouta s vjerojatnošću isključivanja $1-p=0.2$, prema \citep{Huang:2016:DCCN}, loše utječe na performansu modela. Vjerojatnost isključivanja $0.1$ isto ima loš utjecaj na rezultat u odnosu na mrežu bez \textit{dropouta}. 

Unatoč lošoj performansi, za eksperimente je korištena mreža s vjerojatnošću isključivanja $0.2$ kako aposteriorna razdioba parametara ne bi imala premalu entropiju i različiti uzorci mreže s dropoutom ne bi bili previše slični, što bi onemogućilo procjenjivanje epistemičke nesigurnosti predikcije, koja proizlazi iz nesigurnosti u parametre. Kako nije bilo dovoljno memorije za učenje mreže s dropoutom na slikama iz Cityscapesa umanjenim na dimenzije $1024\times448$, kao kod \citet{Kreso:2017:LSDFSSLNI}, pri učenju je korišteno nasumično izrezivanje dijelova slika dimenzija $448\times448$ i broj epoha je povećan s $30$ na $80$. Kod CamVid-a su slike dimenzija $480\times360$ i bilo je dovoljno memorije. Veličina mini-grupe za CamVid je bila $8$.



Početni cilj je bio reproducirati rezultate za semantičku segmentaciju iz \citet{Kendall:2017:WUNBDLCV}.
...
Isprobani su postupci koji su opisani u odjeljku~\ref{subsec:pristupi-ood-softmaks-logiti}, tj. prepoznavanje izvanrazdiobnih primjera na temelju maksimalnog izlaza softmaksa \citep{Hendrycks:2016:BDMOODE} i postupak predložen u \cite{Liang:2017:PDOODENN}. Uz to su isprobani ekvivalentni postupci kod kojih se klasifikacija provodi na temelju maksimalnog logita.

\subsubsection{Modeli}

Za ispitivanje su korištene rezidualna mreža WRN-28-10 \citep{Zagoruyko:2016:WRN} i mreža DenseNetBC \citep{Huang:2016:DCCN} s dubinom $L=100$ i faktorom rasta $k=12$, koja će biti označavana s \textit{DN-100-12}. Ni kod jedne mreže se ne koristi \textit{dropout}. Korištene su vlastite implementacije tih mreža koje se ne bi trebale previše razlikovati od originalnih, tj. trebalo bi biti sve isto kao što je opisano u \citet{Zagoruyko:2016:WRN,Huang:2016:DCCN}. I jedna i druga ipak postižu malo manju točnost od originalnih mreža na skupu za testiranje skupa CIFAR-10 \citep{Krizhevsky:2009:LMLFTI}. Od mreža korištenih za ispitivanje, WRN-28-10 ima točnost oko $0.957$, a DN-100-12 oko $0.948$.

\subsubsection{Izvanrazdiobni skupovi}

Za ove eksperimente, kao unutarrazdiobni skup korištene su slike iz skupa za testiranje skupa CIFAR-10, kojih ima $10000$, korištene su kao unutarrazdiobni skup. Za izvanrazdiobne skupove su, slično kao kod \cite{Hendrycks:2016:BDMOODE,Liang:2017:PDOODENN}, korišteni skupovi:
\begin{enumerate}
	\item TinyImageNet\footnote{\url{https://tiny-imagenet.herokuapp.com/}} --- podskup ImageNet-a \citep{Deng:2009:ILSHID} sa slikama iz $200$ klasa. Za ove eksperimente su iz skupa za testiranje, koji ima $10000$ slika, konstruirana dva skupa: TinyImageNet-C, kojeg čine slike iz kojih su nasumično izrezani dijelovi dimenzija $32\times 32$, i TinyImageNet-R, kojeg čine slike umanjene na iste dimenzije, tj. dimenzije slika iz skupa CIFAR-10.
...
\end{enumerate}
Kao i kod učenja, za svaki skup, slike koje se daju kao ulaz mreži su normalizirane oduzimanjem prosječne vrijednosti i dijeljenjem sa standardnom devijacijom svake komponente (RGB) u skupu za učenje (i validaciju).

\subsubsection{Izvanrazdiobni skupovi}



$\epsilon\in\cbr{0,10^{-3},2\cdot 10^{-3},\bidot,12\cdot 10^{-3}}$
...
\begin{tabular}{llllll}
	...
	\midrule
	TinyImagenet-C &  $\begin{matrix}50.2&34.8&37.7&\mathbf{34.4}\end{matrix}$ &  $\begin{matrix}91.0&\mathbf{93.4}&91.9&92.1\end{matrix}$ &  $\begin{matrix}92.7&\mathbf{94.3}&92.1&92.0\end{matrix}$ &  $\begin{matrix}87.8&\mathbf{92.0}&90.9&91.8\end{matrix}$ &  $\begin{matrix}0.0&0.0&8.0&8.0\end{matrix}$ \\
	TinyImagenet-R &  $\begin{matrix}55.0&\mathbf{43.2}&47.8&44.1\end{matrix}$ &  $\begin{matrix}89.4&\mathbf{90.2}&88.7&88.1\end{matrix}$ &  $\begin{matrix}\mathbf{90.3}&90.1&88.5&87.0\end{matrix}$ &  $\begin{matrix}86.0&\mathbf{88.8}&87.1&87.7\end{matrix}$ &  $\begin{matrix}0.0&0.0&4.0&4.0\end{matrix}$ \\
	iSUN           &  $\begin{matrix}51.0&31.7&30.4&\mathbf{28.6}\end{matrix}$ &  $\begin{matrix}91.6&\mathbf{93.9}&92.6&92.5\end{matrix}$ &  $\begin{matrix}99.2&\mathbf{99.4}&99.2&99.1\end{matrix}$ &  $\begin{matrix}44.4&60.3&60.7&\mathbf{66.1}\end{matrix}$ &  $\begin{matrix}0.0&0.0&9.0&9.0\end{matrix}$ \\
	Gaussian       &  $\begin{matrix}88.2&71.8&\mathbf{0.0}&0.0\end{matrix}$ &  $\begin{matrix}87.4&91.6&98.6&\mathbf{99.2}\end{matrix}$ &  $\begin{matrix}92.6&95.0&99.1&\mathbf{99.5}\end{matrix}$ &  $\begin{matrix}75.1&82.5&96.6&\mathbf{98.5}\end{matrix}$ &  $\begin{matrix}0.0&0.0&12.0&12.0\end{matrix}$ \\
	Uniform        &  $\begin{matrix}88.2&71.8&\mathbf{0.0}&0.0\end{matrix}$ &  $\begin{matrix}87.4&91.6&98.6&\mathbf{99.2}\end{matrix}$ &  $\begin{matrix}92.6&95.0&99.1&\mathbf{99.5}\end{matrix}$ &  $\begin{matrix}75.1&82.5&96.6&\mathbf{98.5}\end{matrix}$ &  $\begin{matrix}0.0&0.0&12.0&12.0\end{matrix}$ \\
	\bottomrule
\end{tabular}
}
\caption{WRN-28-10, CIFAR-10,  $A = 0.9571$, $T=1000$, (softmax logits softmax-perturb logits-perturb).}
...
\begin{tabular}{llllll}
	...
	{} &                                $\mathit{FPR}_{R=0.95}/\%$ &                                       $\mathit{AUROC}/\%$ &                                $\mathit{AP}_\text{in}/\%$ &                               $\mathit{AP}_\text{out}/\%$ &                             $\epsilon/10^{-3}$ \\
	\midrule
	TinyImagenet-C &  $\begin{matrix}52.3&24.0&30.3&\mathbf{19.9}\end{matrix}$ &  $\begin{matrix}92.5&\mathbf{96.0}&93.6&96.0\end{matrix}$ &  $\begin{matrix}94.7&\mathbf{96.9}&94.6&96.4\end{matrix}$ &  $\begin{matrix}88.8&94.8&93.1&\mathbf{95.6}\end{matrix}$ &  $\begin{matrix}0.0&0.0&8.0&8.0\end{matrix}$ \\
	TinyImagenet-R &  $\begin{matrix}58.5&39.6&47.4&\mathbf{36.3}\end{matrix}$ &  $\begin{matrix}90.3&92.4&91.2&\mathbf{92.7}\end{matrix}$ &  $\begin{matrix}92.6&\mathbf{93.6}&92.8&93.4\end{matrix}$ &  $\begin{matrix}86.2&90.6&89.2&\mathbf{91.5}\end{matrix}$ &  $\begin{matrix}0.0&0.0&4.0&4.0\end{matrix}$ \\
	iSUN           &  $\begin{matrix}51.5&17.8&22.9&\mathbf{11.2}\end{matrix}$ &  $\begin{matrix}93.0&97.1&96.2&\mathbf{98.1}\end{matrix}$ &  $\begin{matrix}99.4&99.8&99.7&\mathbf{99.8}\end{matrix}$ &  $\begin{matrix}46.4&72.8&71.5&\mathbf{83.2}\end{matrix}$ &  $\begin{matrix}0.0&0.0&6.0&6.0\end{matrix}$ \\
	Gaussian       &  $\begin{matrix}74.3&6.1&\mathbf{0.0}&0.0\end{matrix}$ &  $\begin{matrix}93.3&97.0&99.3&\mathbf{99.6}\end{matrix}$ &  $\begin{matrix}96.2&98.3&99.5&\mathbf{99.7}\end{matrix}$ &  $\begin{matrix}85.2&91.2&\mathbf{99.1}&98.8\end{matrix}$ &  $\begin{matrix}0.0&0.0&4.0&4.0\end{matrix}$ \\
	Uniform        &  $\begin{matrix}74.3&6.1&\mathbf{0.0}&0.0\end{matrix}$ &  $\begin{matrix}93.3&97.0&99.3&\mathbf{99.6}\end{matrix}$ &  $\begin{matrix}96.2&98.3&99.5&\mathbf{99.7}\end{matrix}$ &  $\begin{matrix}85.2&91.2&\mathbf{99.1}&98.8\end{matrix}$ &  $\begin{matrix}0.0&0.0&4.0&4.0\end{matrix}$ \\
	\bottomrule
\end{tabular}
}
\caption{DN-100-12, CIFAR-10, $A = 0.9481$, $T=1000$ (softmax logits softmax-perturb logits-perturb).}
