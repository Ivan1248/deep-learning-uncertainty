...
%\izvornik
\includepdf[page={-},offset=0mm 0mm, angle=-0.1]{izvornik}


% Dodavanje zahvale ili prazne stranice. Ako ne želite dodati zahvalu, naredbu ostavite radi prazne stranice.
\zahvala{
Zahvaljujem prof. dr. sc. Siniši Šegviću na pomoći, savjetima i prijedlozima tijekom studiranja i tijekom rada na diplomskom radu. Također zahvaljujem asistentima Ivanu Kreši, Marinu Oršiću i Petri Bevandić na pomoći. Hvala mojoj obitelji na velikoj podršci tijekom studiranja.}
...
U poglavlju~\ref{chap:duboko-ucenje-i-konvolucijske-mreze} su opisani neki od osnovnih pojmova dubokog učenja, posebno u vezi unaprijednih neuronskih mreža i konvolucijskih mreža.
Poglavlje \ref{chap:procjenjivanje-nesigurnosti} obrađuje glavnu temu ovog rada --- procjenu nesigurnosti kod dubokih nadziranih modela. U njemu je opisana podjela nesigurnosti, bayesovske neuronske mreže, mjere za izražavanje nesigurnosti i neki primjeri pristupa za procjenu nesigurnosti i pristupa za prepoznavanje primjera koji su izvan razdiobe skupa za učenje ili prepoznavanje krive klasifikacije. Neki od tih pristupa \citep{Kendall:2017:WUNBDLCV,Hendrycks:2016:BDMOODE,Liang:2017:PDOODENN} su isprobani za ovaj rad. Predložena su i neka poboljšanja postupaka za prepoznavanje primjera koji su izvan razdiobe skupa za učenje. Detalji o eksperimentima i ispitivanim postupcima i rezultati eksperimenata opisani su u poglavlju~\ref{chap:eksperimenti}.

Rad je pisan u \LaTeX-u. Za slike u radu korišteni su TikZ, Matplotlib (i druge biblioteke u Pythonu), Inkscape i Paint.NET. Za programsku potporu korišten je programski jezik Python i biblioteke TensorFlow, NumPy, PyTorch, Scikit-image, Scikit-learn, Matplotlib, SciPy i druge. Izvorni kod za sve je u repozitoriju \url{https://github.com/Ivan1248/deep-learning-uncertainty}.
...
Neizvjesnost neke pojave modeliramo \emphasize{slučajnom varijablom}. Slučajnoj varijabli je dodijeljena \emphasize{razdioba} koja definira skup vrijednosti koje slučajna varijabla može poprimiti i vjerojatnosti ostvarivanja tih vrijednosti. Skup mogućih vrijednosti neke slučajne varijable još nazivamo i \emphasize{prostor elementarnih događaja}. \emphasize{Elementarni događaj} je element prostora elementarnih događaja. Ako je $\rvar x$ slučajna varijabla za koju se u nekom eksperimentu opaža vrijednost $x$, taj događaj ima zapis $\cbr{\rvar x = x}$, a njegovu vjerojatnost označavamo $\P(\cbr{\rvar x = x})$, $\P(\rvar x = x)$ ili $\P(x)$. \emphasize{Događaj} je skup vrijednosti i obično se izražava predikatom nad slučajnom varijablom: $\cbr{R(\rvar x)}\coloneqq\cbr{x\colon R(x)}$. Ako je $\set X$ prostor elementarnih događaja slučajne varijable $\rvar x$, onda $\P(\rvar x\in \set X)=1$. Funkcija 
\begin{align*}
...
\end{align}
gdje je $\set X$ prostor elementarnih događaja slučajne varijable $\rvar x$. Diracovu funkciju možemo promatrati kao limes funkcije gustoće Gaussove razdiobe:
\begin{align*}
\dirac(x)=\lim_{\sigma\to 0}\frac{1}{\sqrt{2\pi}\sigma}\exp\del{\frac{x^2}{2\sigma^2}}.
\end{align*}
Ako je argument Diracove funkcije vektor $\vec x=(x_1,..,x_n)$, onda Diracovu funkciju definiramo umnoškom Diracovih funkcija njegovih elemenata:
...
Združenu vjerojatnost možemo rastaviti rastaviti na faktore prema \emphasize{pravilu umnoška}: 
...
Slučajne varijable $\rvar x$ i $\rvar y$, koje mogu biti zavisne, su uz znanje o ishodu slučajne varijable $\rvar z$ \emphasize{uvjetno nezavisne}, što označavamo s $\rvar x\perp\rvar y\mid\rvar z$, akko su slučajne varijable $\del{\rvar x\mid\rvar z=z}$ i $\del{\rvar y\mid\rvar z=z}$ nezavisne za svaki mogući ishod $z$. Onda za svaku trojku $(x,y,z)$ vrijedi
\begin{align}
...
Entropija kontinuirane slučajne varijable je beskonačna. Ako se u izrazu \eqref{eq:entropy} vjerojatnost zamijeni gustoćom vjerojatnosti, onda sse dobije izraz za \emphasize{diferencijalnu entropiju}:
...
\emphasize{Unakrsna entropija} je očekivanje duljine poruke kodirane optimalnim kodom za razdiobu $\P(\rvar y)$ ako izvor poruka ima razdiobu $\P(\rvar x)$. Ovako je definirana:
\begin{align}
\H_{\rvar y}(\rvar x)\coloneqq \E_{\rvar x}\I(\rvar y=x) = -\E_{\rvar x}\log_b P_{\rvar y}(x) \text{.}
\end{align}
Prema Gibbsovoj nejednakosti\footnote{\url{https://en.wikipedia.org/wiki/Gibbs'_inequality}} vrijedi $\H_{\rvar y}(\rvar x)\geq\H_{\rvar x}(\rvar x)=\H(\rvar x)$. Za unakrsnu entropiju se često koristi oznaka $\H(\rvar x,\rvar y)$, ali ista oznaka se koristi i za entropiju para slučajnih varijabli $(\rvar x,\rvar y)$. Po uzoru na \citet{Olah:2015:VIT}, ovdje koristimo oznaku $\H_{\rvar y}(\rvar x)$.
...
\end{align}
Ona je jednaka KL-divergenciji između razdiobe $\P(\rvar x,\rvar y)$ i razdiobe koja pretpostavlja iste nezavisnost varijabli $\rvar x$ i $\rvar y$ uz iste marginalne razdiobe. Budući da je KL-divergencija nenegativna, međusobna informacija je isto nenegativna. Međusobna informacija se može i na ove načine izraziti:
\begin{align}
\I(\rvar x;\rvar y)
...
\end{align}
gdje je $\beta\in\intco{0,1}$ faktor koji određuje \textit{otpor} proporcionalan \textit{brzini} $\vec v$, tj. otpor je proporcionalan faktoru $(1-\beta)$, što se bolje vidi ako se jednadžba~\eqref{eq:gs-inercija-v} izrazi ovako: 
\begin{align}
\vec v_i = \vec v_{i-1} - (1-\beta)\vec v_{i-1} + \nabla_{\vec x}f(\vec x_{i-1}) \text{.}
\end{align}
$\beta$ se obično odabire da bude bliže $1$. Uz $\beta=1$ dobiva se obični gradijentni spust, a uz $\beta=0$ čestica koja klizi bez otpora i ne gubi energiju. Uz dobro odabran $\beta$ prigušuju se pomaci koji nisu u smjeru gibanja $\vec v$. To omogućuje bržu konvergenciju i izbjegavanje malih lokalnih optimuma i drugih stacionarnih točaka. Svojstva gradijentnog spusta s inercijom su dobro objašnjena u \citet{GOH:2017:WMRW}.

Kada bi gradijent $\vec g$ u svim iteracijama bio konstantan, za brzinu bi nakon velikog broja iteracija prema jednadžbi~\eqref{eq:gs-inercija-v} vrijedilo $\vec v_\infty = \beta\vec v_\infty + \vec g$, tj. $\vec v_\infty = \frac{1}{1-\beta}\vec g$. Kada bismo htjeli da bude $\vec v_\infty = \vec g$, definirali bismo ažuriranje brzine kao pokretni prosjek u jednadžbi~\eqref{eq:adam-azurianje-brzine}.
...
\begin{align}
\vec g_i &= \nabla_{\vec x}f(\vec x_{i-1}), \\
\vec v_i &= \beta_1\vec v_{i-1} + (1-\beta_1)\vec g_i, \label{eq:adam-azurianje-brzine} \\
...
\end{align}
gdje je $\vec v_i$ pokretni prosjek gradijenta (procjena prvog momenta gradijenta), $\vec r_i$ pokretni prosjek kvadrata gradijenta po elementima (procjena drugog momenta gradijenta), a $\epsilon$ mala konstanta. Parametar $\beta_1$ ima ulogu kao $\beta$ kao gradijentnog spusta s inercijom, a $\beta_2$ kao $\rho$ kod RMSPropa. Brzina $\vec v_i$ i pokretni prosjek kvadrata $\vec r_i$ se inicijaliziraju u $\cvec 0$ i u svakom koraku se skaliraju obrnuto proporcionalno udjelu gradijenta u odnosu na inicijalnu vrijednost $\cvec 0$ u pokretnom prosjeku radi poništavanja pristranosti procjena. Za faktore skaliranja vrijedi $\lim_{i\to\infty}\frac{1}{1-\beta^i}=1$.
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{Prikaz usmjerenog grafičkog modela s faktorizacijom $\p(x_1,x_2,x_3,x_4)=\p(x_1)\p(x_2\mid x_1)\p(x_3\mid x_1,x_2)\p(x_4\mid x_1,x_2,x_3)$.}
...
\end{solidenumerate}
Kažemo da skup čvorova $\set E$ d-odvaja čvorove $\rvar x$ i $\rvar y$ akko su sve staze između njih d-odvojene. Vrijedi $\rvar x\perp\rvar y \mid \set E$ akko skup čvorova $\set E$ d-odvaja čvorove $\rvar x$ i $\rvar y$. To se može poopćiti na skupove čvorova. Skup čvorova opažanjem kojega neki čvor ili skup čvorova postaje neovisan o ostatku grafa naziva se \emphasize{Markovljev pokrivač} (engl. \textit{Markov blanket}) tog čvora ili skupa čvorova. Markovljev pokrivač čvora $\rvar x$ je
\begin{align}
...
%\subsection{Neusmjereni grafički modeli}

%Kod neusmjerenih grafičkih modela bridovi  združena razdioba izražava umnoškom 
\textit{}
...
\subsection{Bayesovsko zaključivanje} \label{subsec:bayesovski procjenitelj}

Prethodno opisani procjenitelji daju točkastu procjenu parametara i ne izražavaju nesigurnost procjene kojoj uzrok može biti npr. nedovoljna količina podataka ili šum u podacima za učenje. Kod \emphasize{bayesovskog zaključivanja} parametre promatramo kao slučajne varijable. Prema jednadžbi~\eqref{eq:posterior-bayes}, procjeni parametara odgovara aposteriorna razdioba nad parametrima/hipotezama $\p(\rvec\theta\mid\set D)$. Za nju je integriranjem po svim mogućim parametrima potrebno izračunati marginalnu izglednost $\p(\set{D})=\int\p(\set{D}\mid\vec\theta')\p(\vec\theta')\dif{\vec\theta'}$ u nazivniku. Kod složenijih modela često ne možemo odabrati konjugatnu apriornu razdiobu, a i funkcija izglednosti je sama po sebi već dovoljno složena da se, neovisno o apriornoj razdiobi, marginalna izglednost $\p(\set{D})$ ne može ni analitički ni numerički traktabilno računati, pa je potrebno koristiti aproksimacije.

Vjerojatnost nekog primjera $\vec d$ procjenjujemo marginalizacijom po parametrima:
\begin{align}
\p(\vec d\mid\set D) 
= \int\p(\vec d\mid\vec\theta)\p(\vec\theta\mid\set D) \dif{\vec\theta}
= \E_{\rvec\theta\mid\set D}\p(\vec d\mid\vec\theta) \text{,}
\end{align}
gdje je korištena uvjetna nezavisnost $\rvec d\perp\rset D\mid\rvec\theta$ (zbog čega $\p(\vec d\mid\vec\theta,\rset D)=\p(\vec d\mid\vec\theta)$). Točkasta procjena parametara može se interpretirati kao aproksimacija aposteriorne razdiobe:
\begin{align}
\p(\vec\theta\mid\set{D}) &\approx \dirac(\hat{\vec\theta}-\vec\theta) \text{,} \\
\p(\vec d\mid\set D)
...
\section{Aproksimacijsko zaključivanje} \label{sec:aproksimacija-razdioba}
...
Ovaj pododjeljak se temelji na \citet{Murray:2005:NEBOR} i \citet[odjeljak 5.3]{Murphy:2012:MLPP}.
...
\begin{align*}
\fullfunction{\phi}{\R^n}{\R^m}{\vec x}{\sbr{\phi_1(\vec x),\bidot,\phi_m(\vec x)}^\tp}
\end{align*}
...
\begin{align*}
h(\vec x;\vec W) &= \softmax(\vec W\phi(\vec x)), \\
...
\end{solidenumerate}
Funkcije gubitka su definirane kao negativna log-izglednost, $L(y, h(\vec x))=-\ln\P(y\mid\vec x,\vec w)$, i konveksne su. Optimalne težine linearne regresije mogu se analitički izračunati, logistička regresija i višeklasna logistička regresija se obično uče optimizacijskim postupcima temeljenim na gradijentu.
...
\begin{figure}
	...
	\node[nrect] (w1) [right=of x1] {$w_1\phbox$};
	\node[nrect] (w2) [below=of w1] {$w_2\phbox$};
	\node[nrect,draw=none] (wdots) [below=of w2] {};
	\node[nrect] (wn) [below=of wdots] {$w_n\phbox$};
	...
	\node[above=0 of f] {$y$};
	\end{tikzpicture}
	\caption{Grafički prikaz umjetnog neurona. $\phbox$ predstavlja ulaz čvora, $w\phbox$ umnožak ulaza s težinom $w$.}
...
Na ovaj pododjeljak ima utjecaj \citet{Olah:2015:CCGB}.
...
\begin{align}
\pd{E}{\vec\theta} 
= \sum_{i} \pd{L_i}{\vec\theta} + \pd{}{\vec\theta}R_\text{R}\del{\vec\theta} \text{,}
\end{align}
...
U tablici~\ref{tab:derivacije} prikazane su parcijalne derivacije (Jakobijeve matrice) nekih operacija s obzirom na njihove ulaze. Korištenjem pravila deriviranja kompozicije funkcija mogu se izračunati derivacije složenijih funkcija. Radi efikasnosti se izračunavanje vrijednosti u računskom grafu i algoritam propagacije pogreške unatrag provodi paralelno za više ulaza odjednom. Izvodi gradijenata nekih operacija uz višestruke ulaze (grupe) mogu vidjeti npr. ovdje: \url{http://www.zemris.fer.hr/~ssegvic/du/lab0.shtml}. 
...
\begin{table}
	...
	\end{tabular}
	\endgroup
	\caption{Parcijalne derivacije (Jakobijeve matrice) nekih operacija po njihovim ulazima. Zadnja sva retke predstavljaju gubitak unakrsne entropije (negativnu log-izglednosti) za binarnu i višeklasnu klasifikaciju, gdje je $t$ indeks ciljne klase. $\cvec e_t$ označava jednojedinični vektor s elementima ${\cvec e_j}_\ind{i}\coloneqq\enbbracket{i=j}$.}
...
Kako bi optimizacijski algoritam konvergirao, treba se smanjivati stopa učenja ovisno o iteraciji (epohi). Prema \citet{Goodfellow:2016:DL}, dovoljan uvjet za konvergenciju gradijentnog spusta je
\begin{align}
...
Heuristike korištene za inicijalizaciju težina se temelje na aproksimiranju mreže nizom matričnih množenja i postizanju da varijance gradijenata (i izlaza) budu otprilike konstantne kroz mrežu \citep{Goodfellow:2016:DL}. Ako pretpostavimo da je aktivacijska funkcija linearna, otprilike konstantna varijanca izlaza slojeva može se ostvariti inicijalizacijom slučajnim vrijednostima iz Guassove ili unifomne razdiobe s varijancom $\frac{1}{n}$, gdje je $n$ broj ulaza. \citet{Glorot:2010:UDTDFNN} kao kompromis između jednake varijance gradijenta i jednake varijance izlaza slojeva predlažu varijancu $\frac{2}{n+m}$, gdje je $m$ broj izlaza.
...
\begin{figure}
	...
	\begin{subfigure}[t]{0.48\textwidth}
		...
		\begin{tikzpicture}
		\node[nrect] (x) [] {$\vec X$};
		\node[nrect] (s) [above right=0mm and 5mm of x] {$s(\phbox)$};
		\node[nrect] (m) [above=of s] {$m(\phbox)$};
...
Ovaj odjeljak se temelji na \citet{Grubisic:2018:IPM}.
...
\begin{figure}
	...
	\end{tabular}
	\caption{Prilagođeni prikaz dobivanja neprijateljskog primjera FGSM-om iz \citet{Goodfellow:2014:EHAE}. Nakošene riječi predstavljaju klase, a brojevi u zagradama vjerojatnosti koje im mreža dodjeljuje.}
...
Neprijateljski primjeri mogu se pronaći iterativnim optimizacijskim postupcima prvog reda uz održavanje ograničenja susjedstva. Pokazuje se da je neprijateljske primjere moguće pronaći već samo jednim korakom u smjeru predznaka gradijenta. Jedna vrsta takvog napada je \emph{\textit}{fast gradient sign method} (FGSM) \citet{Goodfellow:2014:EHAE}. Neprijateljskom primjeru koji se pronalazi FGSM-om odgovara sljedeći izraz:
...
Dijeljenje ili višestruka uporaba parametara i dijelova mreže je korisna kada se za podatke vrijede neka svojstva ekvivarijantnosti (npr. na pomak u prostoru ili vremenu, skaliranje, kompozitnost,...). Ono se može primjenjivati i kod višezadaćnog učenja, gdje se dio mreže koristi za dobivanje značajki koje su korisne za učenje većeg broja različitih zadataka.
...
\begin{figure}
	...
	\begin{subfigure}[t]{1\textwidth}
		...
		\begin{tikzpicture}
		\node[nrect] (x) {$\vec x$};
		\node[nrect] (f1) [above right=of x] {$f(\phbox;\vec\theta_1)$};
		\node[nrect] (p1) [below right=of f1] {$+$};
		
		\node[nrect] (f2) [above right=of p1] {$f(\phbox;\vec\theta_2)$};
		\node[nrect] (p2) [below right=of f2] {$+$};
		\node[nrect,draw=none] (dots) [right=of p2] {$\cdots$};	
		\node[nrect] (pn-1) [right=of dots] {$+$};
		
		\node[nrect] (fn) [above right=of pn-1] {$f(\phbox;\vec\theta_n)$};
...
\begin{subfigure}[t]{0.48\textwidth}
	...
	\foreach \i in {1,2,...,\n} {
	  \node[nrect] (y\i) [right=20mm of x\i] {$\sum_{i=1}^3 \braket{\phbox[i]}{\vec w_i}$};
...
\begin{figure}
	...
	\foreach \i in {1,2,...,\n} {
		...
		\node[nrect] (y\i) [right=of xx\i] {$\braket{\phbox}{\vec w}$};
		\node[right=0 of y\i] {$y_\i$};
	}
	\foreach \i in {1,2,...,\n} {
		\foreach \j in {1,2,3}
...
Za smanjivanje mapa značajki često se koriste i uobičajeni postupci interpolacije\footnote{\url{https://en.wikipedia.org/wiki/Multivariate_interpolation}} slika kao što su \emphasize{postupak najbližeg susjeda} i \emphasize{bilinearna interpolacija}.



\chapter{Procjena nesigurnosti kod dubokih modela} \label{chap:procjenjivanje-nesigurnosti}
...
\begin{figure}
	...
	\end{tikzpicture}
	\caption{Model regresije (prema onom na slici~\ref{fig:bm-regresija}) kod kojeg su $\rvec\theta$ nepoznati parametri, $\rvec x$ opaženi ulaz, $\rvar y$ nepoznati izlaz, a $\rvar\epsilon$ šum koji ovisi o ulazu $\rvec x$. Čvorovi s indeksima $i$ predstavljaju podatke za učenje (opaženi čvorovi) i odgovarajući šum ($\epsilon_i$).}
...
Jedan poseban slučaj epistemičke nesigurnosti je nesigurnost u to \emphasize{pripada li primjer razdiobi skupa za učenje}. Modelu možemo dati ulazni primjer za koji ne postoji točna oznaka i nije sličan primjerima u skupu za učenje. Takav primjer može biti npr. slika koja pripada nekoj klasi koja nije među onima koje model treba raspoznavati ili samo slučajni šum. Primjeri koji su izvan razdiobe skupa za učenje dalje ćemo nazivati \emphasize{izvanrazdiobni primjeri}. Primjeri koji su iz razdiobe skupa za učenje nazivat ćemo \emphasize{unutarrazdiobni primjeri}.
...
\end{equation}
marginalna izglednost koja se računa marginalizacijom brojnika po parametrima. Ta marginalizacija ovdje predstavlja glavni problem i aposteriorna razdioba se mora aproksimirati postupcima kao što su oni navedeni u odjeljku~\ref{sec:aproksimacija-razdioba}. Na temelju ulaza i aposteriorne razdiobe parametara provodi se zaključivanje o izlazu (ponovljena jednadžba~\eqref{eq:zakljucivanje-y}):
...
\text{,}
\end{align}
Varijacijske razdiobe koje su definirane varijacijskim parametrima $\vec\phi$ ovdje su predstavljene slučajnim varijablama $\tilde{\rvec\theta}$. Minimizacija s obzirom na parametre varijacijske razdiobe je ekvivalentna maksimizaciji donje granice  marginalne log-izglednosti
\begin{align}
...
\end{align}
s obzirom na varijacijske parametre. Prvi dio tog izraza potiče maksimizaciju izglednosti parametara na skupu za učenje. Razlika u odnosu na maksimizaciju izglednosti kod običnih dubokih mreža je da se izglednost ne maksimizira po točkastim procjenama parametara, nego po po razdiobama. Drugi dio izraza kažnjava udaljavanje od apriorne razdiobe i služi kao regularizacija.

Zamjenom aposteriorne razdiobe u jednadžbi~\eqref{eq:zakljucivanje-y-bnm} zamjenskom razdiobom $q_{\vec\phi}$, zaključivanje o izlazu mreže postaje
\begin{align} \label{eq:zakljucivanje-y-bnm-q}
\p(\vec y\mid \vec x, \set{D})
\approx \int\p(\vec y\mid \vec x,\vec\theta)q_{\vec\phi}(\vec\theta) \dif{\vec\theta}
= \E_{\tilde{\vec\theta}\sim q_{\vec\phi}}\p(\vec y\mid\vec x,\tilde{\vec\theta}) \text{.}
\end{align}

U pododjeljku~\ref{subsec:bnm-dropout} je opisana aproksimacija bayesovske neuronske mreže pomoću \textit{dropouta}.


\section{Mjere za izražavanje nesigurnosti predikcija} \label{sec:mjere-nesigurnosti}

Osnovne mjere za izražavanje nesigurnosti su \emphasize{vjerojatnost}, \emphasize{entropija} i \emphasize{varijanca}. Pretpostavimo da modeliramo (diskretnu ili kontinuiranu) razdiobu $\p(\rvar y\mid\vec x,\vec\theta)$ ili $\p(\rvar y\mid\vec x,\set D)$ ako se provodi bayesovsko zaključivanje. Jedan način izražavanja nesigurnosti predikcije kod klasifikacije je \emphasize{vjerojatnost klase s najvećom vjerojatnošću}
\begin{align}
\max_k \P(\rvar y=k\mid\vec x,\vec\theta) \text{.}
\end{align}
Nesigurnost se može izraziti i \emphasize{entropijom} izlaza,
\begin{align}
\H(\rvar y\mid\vec x,\vec\theta)=-\E_{\rvar y\mid\vec x,\vec\theta}\ln\P\del{y\mid\vec x,\vec\theta} \text{.}
\end{align}
Ako kod regresije nesigurnost predikcije modeliramo kontinuiranom razdiobom (umjesto točkaste procjene izlaza), prikladna mjera nesigurnosti je \emphasize{diferencijalna entropija},
\begin{align}
\h(\rvar y\mid\vec x,\vec\theta)=-\E_{\rvar y\mid\vec x,\vec\theta}\ln\p\del{y\mid\vec x,\vec\theta} \text{.}
\end{align}
Ako $\del{\rvar y\mid\vec x,\vec\theta}$ ima Gaussovu razdiobu, diferencijalna entropija je monotona funkcija varijance izlazne razdiobe, $\sigma=\D\del{\rvar y\mid\vec x,\vec\theta}$. Diferencijalna entropija Gaussove razdiobe se od $\ln\sigma$ razlikuje za konstantu\footnote{\url{https://proofwiki.org/wiki/Differential_Entropy_of_Gaussian_Distribution}}. Ako kod klasifikacije gledamo samo jednu od više klasa, možemo kao mjeru nesigurnosti (ne)pripadanja toj klasi koristiti i \emphasize{binarnu entropiju}, $-p\ln p-\del{1-p}\ln\del{1-p}$, gdje je $p$ vjerojatnost te klase.

Jedan nedostatak ovih mjera je što ne razlikuju epistemičku i aleatornu nesigurnost. 
%Još jedan mogući nedostatak entropije i maksimalne vjerojatnosti može biti to što npr. razdioba $(0.6,0.2,0.2)$ ima veću entropiju od razdiobe $(0.6,0.4,0)$ iako se može interpretirati da je kod prve razdiobe veća jednoznačnost zaključka o klasi.
...
\subsection{Eksplicitno modeliranje aleatorne nesigurnosti predikcijom varijance logita} \label{subsec:eksplicitno-modeliranje-aleatorne}

\citet{Kendall:2017:WUNBDLCV} za bayesovske neuronske mreže predlažu eksplicitno modeliranje aleatorne nesigurnosti predikcijom varijance kod regresije i varijance logita kod klasifikacije.
...
L(y, h'(\vec x;\vec\theta))=\frac{1}{2\sigma(\vec x;\vec\theta)^2}\del{y-h(\vec x;\vec\theta)}^2+\frac{1}{2}\ln\sigma(\vec x;\vec\theta)^2 \text{,}
\end{align}
gdje $h'(\vec x;\vec\theta)\coloneqq(h(\vec x;\vec\theta),\sigma(\vec x;\vec\theta))$. Pretpostavljamo da je razdioba izlaza $\mathcal{N}(h(\vec x;\vec\theta),\sigma(\vec x;\vec\theta))$, tj. $h(\vec x;\vec\theta)$ je predikcija očekivanja izlaza, $\sigma(\vec x;\vec\theta)$ predikcija aleatorne varijance. Takav gubitak potiče predikciju veće varijance kada se očekuje veća kvadratna pogreška. Veća predikcija varijance ublažava kvadratnu pogrešku, a povećava drugi član gubitka. Radi numeričke stabilnosti, \citet{Kendall:2017:WUNBDLCV} predlažu da se umjesto $\sigma(\vec x;\vec\theta)$ kao dio predikcije daje $s(\vec x,\vec\theta)\coloneqq\ln\sigma(\vec x;\vec\theta)^2$. Gubitak se onda može ovako izraziti:
\begin{align} \label{eq:aleatorni-gubitak-regresija-numericki-stabilno}
L(y, h'(\vec x;\vec\theta))=\frac{1}{2}\exp(-s(\vec x;\vec\theta))\del{y-h(\vec x;\vec\theta)}^2+\frac{1}{2}s(\vec x;\vec\theta) \text{.}
\end{align}
...
\begin{align}
\E_{\rvec\theta\mid\set D}\sigma(\vec x;\vec\theta) +  \D_{\rvec\theta\mid\set D}h(\vec x;\vec\theta) \text{.}
\end{align}

\citet{Kendall:2017:WUNBDLCV} za procjenu aleatorne nesigurnosti kod klasifikacije predlažu modeliranje vektora logita Gaussovom razdiobom s dijagonalnom kovarijacijskom matricom, tj. $\rvec s \sim \mathcal{N}\del{g(\vec x;\vec\theta),\diag\del{\sigma(\vec x,\vec\theta)^{\odot2}}}$, gdje je $\rvec s$ slučajni vektor koji predstavlja logite, $g$ funkcija koja daje očekivanje, a $\sigma$ funkcija koja daje vektor standardnih devijacija logita. Izlazni vektor vjerojatnosti, za koji vrijedi $h(\vec x;\vec\theta)_\ind{y}=\p(\rvar y=y\mid\vec x,\vec\theta)$, se računa kao očekivanje softmaksa po razdiobi logita:
\begin{align} \label{eq:kendall-hipoteza-klasifikacija}
h(\vec x;\vec\theta) 
= \E\softmax(\rvec s)
= \E_{\vec s\sim\mathcal{N}(g(\vec x;\vec\theta),\diag\del{\sigma(\vec x,\vec\theta)^{\odot2}})}\softmax(\vec s) \text{.}
\end{align}
Za procjenu ukupne nesigurnosti predikcije \citet{Kendall:2017:WUNBDLCV} predlažu entropiju očekivanja predikcije po aposteriornoj razdiobi parametara: $\H\del{\rvar y\mid\vec x,\set D}=\H\del{\E_{\rvec\theta\mid\set D}(\rvar y\mid\vec x,\vec\theta)}$. Za procjenu epistemičke nesigurnosti predlažu prosječnu varijancu predikcije očekivanja logita po aposteriornoj razdiobi:
\begin{align}
...
&=\ln\del{\E_{\vec s\sim\mathcal{N}(g(\vec x),\diag\del{\sigma(\vec x)^2})} \exp\del{\vec s_\ind{y}-\ln\del{\cvec 1^\tp\exp\del{\vec s}}}} \text{,}
...
\begin{figure}
	...
	\resizebox{1\linewidth}{!}{
		...
		\begin{subfigure}[t]{0.22\linewidth}
			...
			\caption{Samantička segmentacija.}
		\end{subfigure}
		\begin{subfigure}[t]{0.22\linewidth}
			\centering
...
\citet{Rawat:2017:APEBDL,Smith:2018:UMUAED}, s ciljem prepoznavanja neprijateljskih primjera, predlažu korištenje međusobne informacije (jednadžbe~\eqref{eq:mutinf}-\eqref{eq:condentropy}) kod bayesovskog zaključivanja kako bi se razlikovale epistemička i aleatorna nesigurnost:
\begin{align}
...
&= \H(\rvar y\mid\vec x,\set D) - \E_{\rvec\theta\mid\set D} \H(\rvar y\mid\vec x,\vec\theta,\set D) \\
&= \H(\rvar y\mid\vec x,\set D) - \E_{\rvec\theta\mid\set D} \H(\rvar y\mid\vec x,\vec\theta) \text{,} \label{eq:mi-epistemicka-nesigurnost}
\end{align}
gdje su korištene definicija uvjetne entropije i pretpostavka $\rvec y\perp\rset D\mid\rvec\theta$, tj. da znanje o podacima ne nosi nove informacije o oznaci ako znamo parametre modela. 
%Pretpostavljamo i $\rvec\theta\perp\rvec x$ jer ne provodimo zaključivanje o parametrima na naučenom modelu. 
Ako nema epistemičke nesigurnosti, tj. nesigurnosti u parametre, izlazne razdiobe $\p(\rvec y\mid\vec x,\vec\theta)$ će biti jednake. Desni član u izrazu~\eqref{eq:mi-epistemicka-nesigurnost} će biti jednak lijevom i međusobna informacija će biti $0$. Ako nema aleatorne nesigurnosti, izlazna razdioba će za svaki parametar $\vec\theta$ iz aposteriorne razdiobe parametara za neki $y$ imati vjerojatnost $1$, tj. entropiju će im biti $0$. Desni član u izrazu~\eqref{eq:mi-epistemicka-nesigurnost} će onda biti $0$, a lijevi može biti veći od $0$ jer je on entropija prosječne hipoteze za ulaz $\vec x$, predikcije različitih parametara iz aposteriorne razdiobe mogu se razlikovati.

Međusobna informacija se može interpretirati kao količina znanja koja se dobije o parametrima ili smanjenje nesigurnosti parametara ako se dobije oznaka $y$ za primjer $\vec x$. To se bolje vidi ako se ona ovako izrazi:
\begin{align}
\I\del{\del{\rvar y\mid\vec x,\set D};\del{\rvec\theta\mid\set D}}
...
\end{align}
Zadnji izraz je razlika entropije aposteriorne razdiobe parametara i očekivanja entropije aposteriorne razdiobe parametara ako se u podatke uključi označeni primjer $\vec x$.

Kao što je i ilustrirano na slici~\ref{fig:entropije}, vrijedi $0\leq\I(\rvar y,\rvec\theta\mid\vec x,\set D)\leq\H(\rvar y\mid\vec x,\set D)$. Oduzimanjem međusobne informacije od entropije marginalizirane izlazne razdiobe možemo dobiti očekivanje entropije izlazne razdiobe kao mjeru aleatorne nesigurnosti:
\begin{align}
\E_{\rvec\theta\mid\set D} \H(\rvar y\mid\vec x,\vec\theta)
...
\citet{Smith:2018:UMUAED} objašnjavaju uspješnost korištenja varijance softmaksa kao mjere epistemičke nesigurnosti u drugim radovima pokazujući da se ona može dobiti kao aproksimacija međusobne informacije razvojem međusobne informacije u Taylorov red.

\citet{Smith:2018:UMUAED} su koristili aproksimaciju bayesovske neuronske mreže pomoću \textit{dropouta}, što je opisano u pododjeljku~\ref{subsec:bnm-dropout}, i koristili su uzorkovanje, tj. \textit{Monte Carlo} aproksimaciju za procjenu izlazne razdiobe i mjera nesigurnosti.


\section{Primjeri pristupa za procjenu nesigurnosti} \label{sec:pristupi-procjena-nesigurnosti}

U ovom odjeljku su opisani primjeri pristupa za procjenu nesigurnosti i prepoznavanje izvanrazdiobnih i krivo klasificiranih primjera bez puno izmjena uobičajenih dubokih modela. U poglavlju~\ref{chap:eksperimenti} prikazani su rezultati nekih eksperimenata s nekima od opisanih pristupa.

\subsection{Aproksimacija bayesovske neuronske mreže pomoću dropouta} \label{subsec:bnm-dropout}

\textit{Dropout}, opisan u pododjeljku~\ref{subsec:dropout}, je postupak koji tijekom učenja nasumično isključuje jedinice i može se interpretirati kao aproksimacija učenja eksponencijalnog broja mreža koje dijele parametre. Pri ispitivanju se obično usrednjavanje aproksimira tako da se, umjesto isključivanja jedinica, izlazi slojeva usrednje skaliranjem težina prema vjerojatnosti neisključivanja. Drugi način usrednjavanja je usrednjavanje izlaza cijele mreže dobivenih uzorkovanjem uz dropout kao pri učenju \citep{Srivastava:2014:DASWPNNO,Gal:2015:DBA}, što se naziva \emphasize{MC-dropout} (\textit{Monte Carlo dropout}). Taj način usrednjavanja je ispravniji i daje bolju performansu \citep{Srivastava:2014:DASWPNNO,Gal:2015:DBA}, ali je manje efikasan jer zahtijeva veći broj uzoraka izlaza uz isključivanje jedinica.

\citet{Gal:2016:BCNNBAVI} učenje s \textit{dropoutom} interpretiraju kao varijacijsko zaključivanje s Bernoullijevom razdiobom kod bayesovske neuronske mreže.
% i pokazuju da korištenje \textit{MC-dropouta} pri testiranju daje bolje poboljšanje performanse u odnosu na obični \textit{dropout} ako je mreža učena na manjem skupu za učenje.
Ako pretpostavimo da \textit{dropout} dolazi iza slojeva linearne transformacije kojima odgovaraju matrice $\vec M_l$, slučajne varijable koje odgovaraju varijacijskoj razdiobi matrice težina su
\begin{align} \label{eq:dropout-matrica-tezina}
\rvec W_l = \diag\del{\rvec z_l}\vec M_l \text{,}
\end{align}
gdje je $l$ indeks linearnog sloja, $\vec M_l$ matrica varijacijskih parametara, a $\rvar z_l$ slučajni vektor čiji elementi su nezavisne slučajne varijable s Bernoullijevom razdiobom s očekivanjem $p$, ako je $1-p$ vjerojatnost isključivanja. Druge parametre, kao što su pomaci, možemo točkasto procjenjivati, što za svaki parametar odgovara razdiobi koja ima samo jednu moguću vrijednost koja je varijacijski parametar.

Kod bayesovskih neuronskih mreža maksimiziramo marginalnu izglednost, što kod diskriminativnih modela odgovara maksimizaciji izraza (ponovljen izraz~\eqref{eq:bnm-marginalna-izglednost}):
\begin{align} \label{eq:bnm-marginalna-izglednost-ponovljeno}
\E_{\tilde{\vec\theta}\sim q_{\vec\phi}}\del{\sum_i \ln\p\del{\vec y_i\midmid\vec x_i,\rvec\theta=\tilde{\vec\theta}}} - \Dkl{q_{\vec\phi}}{\p(\rvec\theta)}
\end{align}
Prvi član tog izraza možemo aproksimirati \textit{Monte Carlo} aproksimacijom. Ako zamijenimo redoslijed očekivanja i zbroja i očekivanje aproksimiramo jednim uzorkom, on se može ovako aproksimirati:
\begin{align}
...
\end{align}
gdje su $\tilde{\vec\theta}_i$ uzorci parametara iz varijacijske razdiobe $q_{\vec\phi}$. Za svaki primjer u nekoj iteraciji uzima se jedan uzorak parametara, kao što je uobičajeno kada se koristi \textit{dropout}. Drugi član u izrazu~\eqref{eq:bnm-marginalna-izglednost-ponovljeno} (bez minusa) je
\begin{align}
...
\end{align}
Prvi integral u zadnjem izrazu je beskonačan zato što se faktori varijacijske razdiobe sastoje od Diracovih \textit{šiljaka}. Šiljke možemo npr. aproksimirati proizvoljno uskim pravokutnicima širine $2\epsilon$ i konačne visine:
...
\end{align}
Ako pretpostavimo da neke težine neće postati točno $0$, tj. bliže nuli od $2\epsilon$, prvi član u izrazu~\eqref{eq:dropoutvi-dkl-qp-2} (diferencijalna entropija varijacijske razdiobe) onda postaje konačan i neovisan o varijacijskim parametrima i može se zanemariti kod učenja. Za drugi član u izrazu~\eqref{eq:dropoutvi-dkl-qp-2} ne aproksimiramo varijacijsku razdiobu. On je težinski zbroj višedimenzionalnih Diracovih \textit{šiljaka} koji \textit{uzorkuju} apriornu razdioba. Ako se koristi Gaussova apriorna razdioba, može se pokazati da je taj član proporcionalan $p$ i zbroju kvadrata težina, što odgovara $L^2$ regularizaciji. Malo drugačiji i detaljniji izvod može se vidjeti u \citet{Gal:2015:DBA,Gal:2015:DBAA}, gdje se mreža s \textit{dropoutom} interpretira kao aproksimacija s Gaussovog procesa\footnote{\url{https://en.wikipedia.org/wiki/Gaussian_process}}. Vidimo da, ako su regularizirane samo matrice težina, maksimizaciji izraza~\eqref{eq:bnm-marginalna-izglednost-ponovljeno} odgovara minimizacija ove funkcije pogreške:
\begin{align}
...
Kao što je spomenuto u odjeljku~\ref{sec:vaznost-primjene-nesigurnosti}, razdiobe koje duboki modeli daju kao izlaz softmaksa su često previše sigurne kod krive klasifikacije i nije ih dobro interpretirati kao vjerojatnosti. Međutim, \citet{Hendrycks:2016:BDMOODE} pokazuju da se krivo klasificirani i izvanrazdiobni primjeri mogu uspješnije nego što je očekivano prepoznavati klasifikacijom maksimalne vjerojatnosti softmaksa kod različitih zadataka i skupova podataka. \citet{Guo:2017:CMNN} pokazuju da se samo \emphasize{temperaturnim skaliranjem} softmaksa, tj. dijeljenjem svih logita s $T$ prije softmaksa, može značajno poboljšati kalibracija\footnote{Model je dobro kalibriran ako svaka vjerojatnost koju dodjeljuje nekoj klasi slična stvarnoj učestalosti te klase među primjerima kojima model dodjeljuje tu vjerojatnost.} već naučene mreže. Kod temperaturnog skaliranja, ako su logiti $\vec s$ ($h(\vec x)=\softmax(\vec s)$), izlazni vektor vjerojatnosti uz temperaturno skaliranje je $\softmax\del{\frac{1}{T}\vec s}$. Optimalni $T$ se traži na validacijskom skupu. \citet{Guo:2017:CMNN} su još isprobali nekoliko složenijih transformacija, ali s njima nisu dobili bolje rezultate. Još su zaključili je da se optimalne općenitije transfomacije logita kao $\softmax(\vec s\oslash\vec t)$, gdje se svaki logit dijeli zasebnim elementom vektora $\vec t$, otprilike svode na temperaturno skaliranje, tj. vrijedi $\vec t^*\approx T^*\cvec 1$, gdje su $\vec t^*$ i $T^*$ optimalni. 

\citet{Liang:2017:PDOODENN} predlažu dva poboljšanja klasifikacije maksimalne vrijednosti softmaksa za prepoznavanje izvanrazdiobnih primjera. Jedno poboljšanje je temperaturno skaliranje. Pokazuju da, što je veća temperatura, to se izvanrazdiobni primjeri mogu bolje odvojiti od unutarrazdiobnih primjera na temelju maksimalne vrijednosti softmaksa. Drugo poboljšanje je izmjena ulaza mreže tako da se FGSM-om (jednadžba~\eqref{eq:FGSM}) pomakne u smjeru povećavanja maksimalnog izlaza softmaksa (za razliku od smanjivanja kod neprijateljskih primjera):
\begin{equation} \label{eq:ODIN-FGSM}
\tilde{\vec x} = \vec x - \epsilon\sgn\nabla_{\vec x}\del{-\ln \max_k \P(\rvar y=k\mid x,\vec\theta)} \text{.}
\end{equation}
$\epsilon$ je parametar koji se određuje pomoću izdvojenog podskupa izvanrazdiobnih primjera. \citet{Liang:2017:PDOODENN} empirijski pokazuju da takav pomak ulaznog primjera ima veći utjecaj na unutarrazdiobne primjere i tako ih bolje razdvaja od izvanrazdiobnih.
...
Kod semantičke segmentacije ulazni primjeri su slike i svakom pikselu se dodjeljuje oznaka. Kod evaluacije se obično pikseli iz svih slika iz skupa za ispitivanje smatraju nezavisnim primjerima, pa je zbroj svih elemenata konfuzijske matrice $\envert{\set D}HW$, gdje je $\envert{\set D}$ broj slika u skupu za ispitivanje, a $H$ i $W$ prostorne dimenzije slika. Uspjeh kod semantičke segmentacije često vrednujemo točnošću i srednjim omjerom presjeka i unije.


\section{Procjena i razlikovanje nesigurnosti kod semantičke segmentacije pomoću MC-dropouta}

Aproksimacija bayesovske neuronske mreže pomoću \textit{dropouta} \citep{Gal:2015:DBA,Gal:2015:DBAA,Gal:2016:BCNNBAVI} je opisana u pododjeljku~\ref{subsec:bnm-dropout}. Neki pristupi \citep{Kendall:2017:WUNBDLCV,Smith:2018:UMUAED} za razlikovanje aleatorne i epistemičke nesigurnosti pomoću \textit{dropouta} su opisani u pododjeljku~\ref{sec:razlikovanje-aleatorne-epistemicke}. \citet{Kendall:2017:WUNBDLCV} su postupak opisan u pododjeljku~\ref{subsec:bnm-dropout} koristili kod konvolucijskih mreža na zadatku semantičke segmentacije, gdje se svakom pikselu dodeljuje oznaka, i na zadatku procjene prostorne dubine (regresije) svakog piksela ulazne slike. 

Kako bi razlikovali aleatornu i epistemičku nesigurnosti kod semantičke segmentacije, \citet{Kendall:2017:WUNBDLCV} aleatornu nesigurnost modeliraju pomoću predikcije varijanci logita svakog piksela kao što je opisano u pododjeljku~\ref{subsec:eksplicitno-modeliranje-aleatorne}. Gubitak za svaki piksel je negativni logaritam vjerojatnosti ciljne klase, ali vjerojatnosti se računaju kao očekivanje izlaza softmaksa po razdiobi logita, kao što je opisano jednadžbom~\eqref{eq:kendall-gubitak-klasifikacija}. Očekivanje se procjenjuje \textit{Monte Carlo} aproksimacijom. Za semantičku segmentaciju se ukupan gubitak jedne slike računa kao srednji gubitak po pikselima. Kao mjeru epistemičke nesigurnosti, \citet{Kendall:2017:WUNBDLCV} za svaki piksel koriste prosječnu varijancu logita uz \textit{MC-dropout}, a aleatornu nesigurnost modeliraju logitima koji imaju Gaussovu razdiobu s dijagonalnom kovarijacijskom matricom, za koju se uči predikcija očekivanja i varijance. Ovaj postupak nije do kraja ostvaren zbog ograničenog vremena i umjesto njega je ispitan jednostavniji postupak s procjenom epistemičke nesigurnosti međusobnom informacijom uz \textit{MC-dropout} prema \citet{Rawat:2017:APEBDL,Smith:2018:UMUAED}, kao što je opisano u pododjeljku~\ref{subsec:mi-epistemicka}. 

Neka $\overline{h}$ označava \textit{Monte Carlo} aproksimaciju hipoteze -- usrednjeni izlaz softmaksa po uzorcima parametara (vektor vjerojatnosti):
\begin{align}
\overline{h}(\vec x) \coloneqq \frac{1}{M}\sum_{i=1}^M h\del{\vec x;\tilde{\vec\theta}_i} \text{,}
\end{align}
gdje je $M$ broj uzoraka za \textit{Monte Carlo} aproksimaciju, a $\tilde{\vec\theta}_i$ uzorci iz varijacijske razdiobe koja odgovara \textit{dropoutu}. Procjena ukupne nesigurnosti predikcije je procjena entropije izlazne razdiobe:
...
\end{align}
Nenegativnost međusobne informacije će biti zadržana i za aproksimaciju, gdje su očekivanja po empirijskoj razdiobi uzoraka parametara $\hat{p}_{\vec\theta}(\vec\theta)=\sum_i \dirac(\vec\theta-\tilde{\vec\theta}_i)$.

Za ovaj rad su ispitane neke od tih ideja na zadatku semantičke segmentacije.

\subsection*{Skupovi podataka}

Za učenje i ispitivanje su korišteni ovi skupovi podataka s oznakama za semantičku segmentaciju:
\begin{solidenumerate}
	\item CamVid\footnote{\url{https://github.com/alexgkendall/SegNet-Tutorial/tree/master/CamVid}} \citep{Brostow:2008:CamVid} -- skup kojeg čine slijedne slike dimenzija $480\times360$ iz snimki vožnje u gradu s oznakama za $32$ klase. Skup za učenje ima $367$ slika, skup za validaciju $101$, a skup za testiranje $233$. CamVid je korišten u \citet{Kendall:2017:WUNBDLCV}.
	\item Cityscapes \citep{Cordts:2016:Cityscapes} -- skup slika dimenzija $2048\times1024$ iz vožnje u gradovima s oznakama za $19$ klasa. Skup za učenje ima $2975$ slika, skup za validaciju $500$, a skup za testiranje $1525$ slika. Nisu dostupne ciljne oznake skupa za testiranje. Za učenje i ispitivanje su korištene slike umanjene uz odsječen donji dio, koji uglavnom činu neoznačena hauba, tako da budu dimenzija $1024\times448$.
	\item WildDash\footnote{\url{http://www.wilddash.cc/}} -- skup slika iz vožnje s različitim vremenskim uvjetima, osvjetljenjima, okolinama, vozilima, rijetkim pojavama i s različitim parametrima kamere. Slike su dimenzija $1920\times1080$. Oznake su iste kao kod Cityscapesa. Za eksperimente je korišten podskup $\text{WildDash}_\text{bench}$ koji nema oznake i sadrži posebno teške slike od kojih su neke izobličene, zašumljene, imaju izmijenjene boje, a neke nisu slike prometa, tj. nisu slične slikama iz razdiobe skupa za učenje i njihovi pikseli ne pripadaju nijednoj klasi.
\end{solidenumerate}

\subsection*{Modeli}

Umjesto modela koji su koristili \citet{Kendall:2017:WUNBDLCV}, korištena je mreža LadderDenseNet-121 \citep{Kreso:2017:LSDFSSLNI}, za koju je potrebno manje memorije. Ona se temelji na mreži DenseNetBC-121 \citep{Huang:2016:DCCN} i prilagođena je za semantičku segmentaciju. Korištena je vlastita implementacija te mreže koja ne postiže jednako dobru performansu kao originalna. Ta mreža će se označavati s \textit{LadderDensenet-121-V}. Kao i kod \citep{Kreso:2017:LSDFSSLNI}, za dio mreže koji čini DenseNet-121 korištena je inicijalizacija parametrima mreže DenseNet-121 učene na ImageNet-u \citep{Deng:2009:ILSHID}.
%Na validacijskom skupu Cityscapesa LadderDensenet-121-V bez \textit{dropouta} postiže srednji omjer presjeka i unije $\mIoU=0.6721(0.007)$ i točnost piksela $A=0.9459(0.0006)$ (sredina i standardna devijacija skupa od $5$ mjerenja), što je značajno manje od $\mIoU=0.7282$ i $A=0.9506$ kod originalne implementacije. Mreža učena na CamVidu uz isti postupak učenja postiže $\mIoU=0.6774(0.0031)$ i $A=0.9176(0.0025)$ na skupu za testiranje CamVida.

Eksperimenti pokazuju da dodavanje \textit{dropouta} s vjerojatnošću isključivanja $1-p=0.2$, kao kod \citep{Huang:2016:DCCN}, loše utječe na performansu kod LadderDenseNet-a. Čini se da ni vjerojatnost isključivanja $0.1$ nema značajno pozitivan utjecaj na rezultat u odnosu na mrežu bez \textit{dropouta} na Cityscapes-u. Kod CamVida, koji je manji skup, dropout ima pozitivan učinak. 

Za eksperimente je, ako nije drugačije navedeno, korištena mreža s vjerojatnošću isključivanja $0.2$ kako aposteriorna razdioba parametara ne bi imala premalu entropiju i različiti uzorci mreže s \textit{dropoutom} ne bi bili previše slični, što bi onemogućilo dobro procjenjivanje epistemičke nesigurnosti predikcije, koja proizlazi iz nesigurnosti u parametre. Kako nije bilo dovoljno memorije za učenje mreže s \textit{dropoutom} na slikama iz Cityscapesa umanjenim na dimenzije $1024\times448$, kao kod \citet{Kreso:2017:LSDFSSLNI}, pri učenju je korišteno nasumično izrezivanje dijelova slika dimenzija $448\times448$ i broj epoha je povećan s $30$ na $80$. Veličina mini-grupe za CamVid je $8$. Za CamVid je, ako nije drugačije navedeno, korišteno $60$ epoha za učenje.

Za \textit{MC-dropout} je kao kod \citet{Kendall:2017:WUNBDLCV} korišteno po $50$ uzoraka za procjenu izlazne razdiobe klasa i nesigurnosti za svaki primjer.

\subsection*{Rezultati}

U tablici~\ref{tab:evaluacija-camvid} prikazani su rezultati evaluacije različitih modela na CamVidu, a u tablici~\ref{tab:evaluacija-cityscapes} rezultati učenja na skupu za učenje i evaluaciji na validacijskom skupu Citysapesa. Vidi se da \textit{MC-dropout} ima pozitivan učinak na evaluaciju u odnosu na obični \textit{dropout}.
...
\begin{table}
	...
	\begin{tabular}{lrr}
		...
		\midrule
		FC-DenseNet + \textit{dropout} \citep{Kendall:2017:WUNBDLCV} & $67.1\phantom{0(0.00)}$ & - \\
		...
		\\\bottomrule
	\end{tabular}
	\caption{Usporedba rezultata evaluacije na skupu CamVid. Vrijednosti za LadderDenseNet-121-V su prosjek $5$ evaluacija.}
...
\begin{table}
	...
	\begin{tabular}{llrrrr}
		...
		\bottomrule
	\end{tabular}
	\caption{Srednje procjene epistemičke nesigurnosti $U_\text{E}$ i aleatorne nesigurnosti $U_\text{E}$ za različite parove skupa za učenje i skupa za ispitivanje i njihovi omjeri. U indeksima su imena podskupova. $1/n$ u indeksu označava da se uzima slučajan podskup s $1/n$ od svih primjera. Svakom skupu za učenje odgovara jedna mreža. Mreže učene na CamVidu učene su u $30$ epoha.}
...
\begin{table}
	\begin{subtable}[t]{1\textwidth}
		...
		\end{tabular}			
		\endgroup
		\caption{Srednji omjer presjeka i unije i točnost za \textit{dropout} i \textit{MC-dropout} u ovisnosti o veličini skupa za učenje i broju epoha. U parovima vrijednosti prva se odnosi na \textit{dropout}, a druga na \textit{MC-dropout}. Svaki par vrijednosti je artimetička sredina evaluacije triju mreža.}
	...
	\begin{subtable}[t]{1\textwidth}
		...
		\begin{tabular}{c rrr}
			\toprule
			\multirowcell{2}{$\alpha$} & $30$ epoha & $60$ epoha & $120$ epoha \\
			\cmidrule{2-4}
			{} 
			& $\hnc{U_\text{E}}\;\hnc{U_\text{A}}\;\hnc{\frac{U_\text{E}}{U_\text{A}}}$ &  $\hnc{U_\text{E}}\;\hnc{U_\text{A}}\;\hnc{\frac{U_\text{E}}{U_\text{A}}}$ & $\hnc{U_\text{E}}\;\hnc{U_\text{A}}\;\hnc{\frac{U_\text{E}}{U_\text{A}}}$
			\\
			\midrule
			$1/1$ 
		...
		\end{tabular}			
		\endgroup
		\caption{Prosječna procjena epistemičke nesigurnosti $U_\text{E}$, prosječna procjena aleatorne nesigurnosti $U_\text{E}$ i njihov omjer u ovisnosti o veličini skupa za učenje i broju epoha. Svaka trojka vrijednosti dobivena je jednim mjerenjem.}
...
Na slikama~\ref{fig:nesigurnost-predikcije-cityscapes}~i~\ref{fig:nesigurnost-predikcije-wilddash} su prikazani odabrani primjeri predikcija i nesigurnosti kod kojih se vidi razlika između procjene aleatorne i procjene epistemičke nesigurnosti. Reci redom predstavljaju:
\begin{solidenumerate}
	\item ulaznu sliku
...
\end{solidenumerate}
Kod slika koje predstavljaju nesigurnost, boje od crne do bijele predstavljaju vrijednosti iz intervala $\intcc{0,\ln C}$, gdje je $C=19$ broj klasa. Jedino za aleatornu nesigurnost, koja je pomnožena s $4$, taj interval je $\intcc{0,\frac{1}{4}\ln C}$ i bijela boja predstavlja sve vrijednosti iz intervala $\intcc{\frac{1}{4}\ln C,\ln C}$. Na slici~\ref{fig:boje-klasa-cityscapesa} se vidi koja boja predstavlja koju klasu kod semantičke segmentacije. Mreža je naučena na skupu za učenje Cityscapesa s \textit{dropoutom}.

\begin{figure}
	\centering\footnotesize
	\begin{minipage}[b]{1.9cm}\offinterlineskip
		\begin{tabu} {@{}X[1,c,m]@{}@{}X[-1,c,m]@{}}
			\makecell[l]{ulaz} & \vspace{1.6cm} \\
			\makecell[l]{ciljna\\ oznaka} & \vspace{1.6cm} \\
			\makecell[l]{predikcija --\\ \textit{dropout}} & \vspace{1.6cm} \\
			\makecell[l]{predikcija --\\ \textit{MC-dropout}} &\vspace{1.6cm} \\
			\makecell[l]{entropija --\\ \textit{dropout}} &\vspace{1.6cm} \\
			\makecell[l]{entropija --\\ \textit{MC-dropout}} &\vspace{1.6cm} \\
			\makecell[l]{aleatorna\\ nesigurnost} &\vspace{1.6cm} \\
			\makecell[l]{epistemička\\ nesigurnost\\$\cdot4$ } &\vspace{1.6cm} \\
		\end{tabu}
	\end{minipage}
	...
	\caption{Predikcije i procjene nesigurnosti na odabranim slikama iz skupova za validaciju i testiranje Cityscapesa na kojima se vide veća razlike između procjena epistemičke i aleatorne nesigurnosti.}
...
\begin{figure}
	...
	\begin{tabu} {@{}X[1,c,m]@{}@{}X[-1,c,m]@{}}
		...
		\makecell[l]{entropija --\\ \textit{MC-dropout}} &\vspace{1.6cm} \\
		\makecell[l]{aleatorna\\ nesigurnost} &\vspace{1.6cm} \\
		\makecell[l]{epistemička\\ nesigurnost\\$\cdot$ $4$ } &\vspace{1.6cm} \\
	\end{tabu}
\end{minipage}
	\raisebox{-.5\height+0.85mm}{\includegraphics[height=11.2cm,keepaspectratio]{experiments/uncertainty-predictions-wilddash.png}}
	\caption{Predikcije i procjene nesigurnosti na odabranim slikama iz skupa WildDash na kojima se vidi veća razlike između procjena epistemičke i aleatorne nesigurnosti.}
...
%eksplicitna aleatorna nesigurnost s Gaussovom razdiobom logita kao u %\citep{Kendall:2017:WUNBDLCV} -- ako stignem (mislim da neću)
...
\begin{solidenumerate}
	\item Stopa lažnih pozitiva $\FPR$ kada je odziv $0.95$, što će biti označavano s $\FPR_{R=0.95}$. $\FPR$ se može interpretirati kao vjerojatnost da se izvanrazdiobni primjer krivo klasificira kao unutarrazdiobni.
...
\subsection*{Odabir parametara}

Na temelju rezultata u \citet{Liang:2017:PDOODENN}, koji pokazuju da su veće temperature bolje, za eksperimente u kojima se koristi temperaturno skaliranje softmaksa korištena je temperatura $T=1000$. Za odabir veličine pomaka $\epsilon$ iz svakog izvanrazdiobnog skupa se izdvajalo $10\%$ slika. 

Optimalni $\epsilon$ je tražen u skupu $\cbr{0,10^{-3},2\cdot 10^{-3},\bidot,10\cdot 10^{-3}}$ tako da minimizira $\FPR$ kad je odziv $0.95$. I kod mreže WRN-28-10 i kod mreže DN-100-12 (vlastitih implementacija), optimalni $\epsilon$ je ispadao otprilike $5$ puta veći nego kod \citet{Liang:2017:PDOODENN} za svaki izvanrazdiobni skup.

Ispitana su još neka poboljšanja koja uzimaju u obzir klasu logita i/ili veći broj značajki izvedenih iz vektora logita za klasifikaciju. U eksperimentima s tim postupcima za učenje parametara je korišteno $20\%$ primjera iz unutarrazdiobnog skupa i iz svakog izvanrazdiobnih skupova. 

\subsection*{Rezultati}

Rezultati glavnih eksperimenata prikazani su u tablicama~\ref{tab:ood-dn-cifar}~i~\ref{tab:ood-wrn-cifar}. U tablicama, za svaku evaluacijsku mjeru $5$ stupaca redom predstavlja klasifikaciju po maksimalnoj vrijednosti:
\begin{solidenumerate}
...
\begin{subtable}[t]{1\textwidth}
	...
	\midrule
	%CIFAR-10       &  $\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}$ &  $\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}$ &  $\hnc{\mathbf{62.1}}\;\hnc{52.6}\;\hnc{52.6}\;\hnc{52.5}\;\hnc{52.6}$ &  $\hnc{\mathbf{47.4}}\;\hnc{\mathbf{47.4}}\;\hnc{\mathbf{47.4}}\;\hnc{\mathbf{47.4}}\;\hnc{\mathbf{47.4}}$ &  $\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}$ \\
...
\begin{subtable}[t]{1\textwidth}
	\resizebox{\textwidth}{!}{%
		...
		\begin{tabular}{lrrrrr}
			...
			{} &                                                                                 $\mathit{FPR}_{R=0.95}/\%$ &                                                                                        $\mathit{AUROC}/\%$ &                                                               $\mathit{AP}/\%$ &                                                                                $\mathit{AP}_\text{n}/\%$ &                                       $\epsilon/10^{-3}$ \\
			\midrule
			%CIFAR-10       &  $\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.1}}\;\hnc{\mathbf{95.1}}\;\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}$ &  $\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{49.9}}$ &  $\hnc{\mathbf{52.9}}\;\hnc{52.6}\;\hnc{52.6}\;\hnc{\mathbf{52.6}}\;\hnc{52.6}$ &  $\hnc{\mathbf{47.3}}\;\hnc{\mathbf{47.3}}\;\hnc{\mathbf{47.3}}\;\hnc{\mathbf{47.3}}\;\hnc{\mathbf{47.3}}$ &  $\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}$ \\
...
S ciljem pokazivanja da se mogu dobiti bolji rezultati kod prepoznavanja izvanrazdiobnih primjera na temelju logita, napravljen je još jedan eksperiment s postupcima koji nadograđuju klasifikaciju maksimalnog logita. Ispitane su sljedeće ideje i njihova međusobna kombinacija i kombinacija s FGSM-om. Jedna ideja je bila normalizirati logite prije uzimanja maksimalne vrijednosti tako da se svakom logitu oduzme njegova sredina i da se podijeli sa standardnom devijacijom na izdvojenom skupu. Druga ideja je bila koristiti više značajki dobivenih iz vektora logita i plitki klasifikator (logističku regresiju s baznim funkcijama) koji na temelju tih značajki određuje je li primjer unutar ili zvan razdiobe skupa za učenje. Kao ulaz klasifikatora korišten je vektor $\vec z=\sbr{\max_i \vec s_\ind{i},\sum_i \vec s_\ind{i}}$. Za klasifikator se koristi logistička regresija s normalizacijom ulaza i polinomijalnim značajkama stupnjeva od $0$ do $2$, tj.
\begin{align}
\sigma(\vec w^\tp\phi(N(\vec z))),
\end{align}
gdje je $N$ funkcija normalizacije, a $\phi\colon\sbr{a,b}\mapsto\sbr{1,a,b,a^2,b^2,ab}$ funkcija polinomijalnih značajki koja omogućuje nelinearnu granicu u ulaznom prostoru klasifikatora. Takva funkcija $\phi$ je prikladna kada su razdiobe klasa slične Gaussovoj razdiobi. Normalizacija se koristi kako $L^2$ regularizacija kod logističke regresije ne bi previše kažnjavala težine nekih značajki u odnosu na druge težine. Korištena je implementacija logističke regresije u biblioteci Scikit-learn s unaprijed zadanim parametrima. Regularizacijski parametar je $C=1$. Računski grafovi ovih i ranijih postupaka prikazani su na slici~\ref{fig:ood-racunski-grafovi}
...
Uspoređeni su postupci prepoznavanja izvanrazdiobnih primjera:
\begin{solidenumerate}
	\item po maksimalnom logitu (slika~\ref{subfig:graf-ood-logiti}) uz FGSM
	\item logističkom regresijom po maksimalnom logitu i zbroju logita (slika~\ref{subfig:graf-ood-logreg})
	\item logističkom regresijom po maksimalnom logitu i zbroju logita uz FGSM
	\item po maksimalnom normaliziranom logitu (slika~\ref{subfig:graf-ood-normalizacija})
	\item po maksimalnom normaliziranom logitu uz FGSM
	\item logističkom regresijom po maksimalnom normaliziranom logitu i zbroju logita (slika~\ref{subfig:graf-ood-normalizacija-logreg})
	\item logističkom regresijom po maksimalnom normaliziranom logitu i zbroju logita uz FGSM.
\end{solidenumerate}
Za učenje normalizacije, parametara logističke regresije $\vec w$ i pomaka FGSM-a $\epsilon$, za unutarrazdiobni i svaki izvanrazdiobni skup izdvojeno je $20\%$ primjera. Svi parametri su učeni posebno za svaki izvanrazdiobni skup. Rezultati su prikazani u tablicama~\ref{tab:ood-dn-cifar-poboljsano}~i~\ref{tab:ood-wrn-cifar-poboljsano}.

\begin{table} \centering
	\begin{subtable}[t]{1\textwidth}
\resizebox{\textwidth}{!}{%
	\begingroup
	\newcommand\hnc[1]{\phantom{\mathbf{100.0}}\mathllap{#1}}
	\begin{tabular}{lrrrrr}
		\toprule
		{} &                                                                                                                           $\mathit{FPR}_{R=0.95}/\%$ &                                                                                                                                  $\mathit{AUROC}/\%$ &                                                                                                            $\mathit{AP}/\%$ &                                                                                                   $\mathit{AP}_\text{n}/\%$ &                                                             $\epsilon/10^{-3}$ \\
		\midrule
		%CIFAR-10       &  $\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}$ &  $\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}$ &  $\hnc{50.0}\;\hnc{\mathbf{75.0}}\;\hnc{\mathbf{75.0}}\;\hnc{50.0}\;\hnc{50.0}\;\hnc{\mathbf{75.0}}\;\hnc{\mathbf{75.0}}$ &  $\hnc{50.0}\;\hnc{\mathbf{75.0}}\;\hnc{\mathbf{75.0}}\;\hnc{50.0}\;\hnc{50.0}\;\hnc{\mathbf{75.0}}\;\hnc{\mathbf{75.0}}$ &  $\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}$ \\
		TinyImageNet-C &  $\hnc{20.4}\;\hnc{24.1}\;\hnc{20.1}\;\hnc{16.5}\;\hnc{15.6}\;\hnc{15.9}\;\hnc{\mathbf{15.0}}$ &  $\hnc{96.0}\;\hnc{96.1}\;\hnc{96.2}\;\hnc{\mathbf{97.1}}\;\hnc{\mathbf{97.2}}\;\hnc{\mathbf{97.2}}\;\hnc{\mathbf{97.3}}$ &  $\hnc{96.1}\;\hnc{96.7}\;\hnc{96.2}\;\hnc{\mathbf{97.5}}\;\hnc{\mathbf{97.4}}\;\hnc{\mathbf{97.6}}\;\hnc{\mathbf{97.6}}$ &  $\hnc{95.9}\;\hnc{95.4}\;\hnc{96.0}\;\hnc{96.8}\;\hnc{\mathbf{97.1}}\;\hnc{\mathbf{96.9}}\;\hnc{\mathbf{97.2}}$ &  $\hnc{6.0}\;\hnc{0.0}\;\hnc{7.0}\;\hnc{0.0}\;\hnc{6.0}\;\hnc{0.0}\;\hnc{4.0}$ \\
		TinyImageNet-R &  $\hnc{37.4}\;\hnc{39.6}\;\hnc{36.6}\;\hnc{33.4}\;\hnc{\mathbf{31.8}}\;\hnc{32.8}\;\hnc{32.8}$ &  $\hnc{92.0}\;\hnc{92.7}\;\hnc{92.8}\;\hnc{\mathbf{93.8}}\;\hnc{\mathbf{93.9}}\;\hnc{\mathbf{93.9}}\;\hnc{\mathbf{93.9}}$ &  $\hnc{91.6}\;\hnc{93.3}\;\hnc{93.1}\;\hnc{\mathbf{94.3}}\;\hnc{\mathbf{94.2}}\;\hnc{\mathbf{94.3}}\;\hnc{\mathbf{94.3}}$ &  $\hnc{91.7}\;\hnc{91.7}\;\hnc{92.4}\;\hnc{93.1}\;\hnc{\mathbf{93.5}}\;\hnc{\mathbf{93.3}}\;\hnc{\mathbf{93.3}}$ &  $\hnc{6.0}\;\hnc{0.0}\;\hnc{4.0}\;\hnc{0.0}\;\hnc{4.0}\;\hnc{0.0}\;\hnc{0.0}$ \\
		LSUN-C         &  $\hnc{32.9}\;\hnc{28.6}\;\hnc{28.6}\;\hnc{23.1}\;\hnc{23.1}\;\hnc{\mathbf{18.1}}\;\hnc{\mathbf{18.1}}$ &  $\hnc{92.7}\;\hnc{95.3}\;\hnc{95.3}\;\hnc{96.3}\;\hnc{96.3}\;\hnc{\mathbf{96.7}}\;\hnc{\mathbf{96.7}}$ &  $\hnc{92.2}\;\hnc{95.8}\;\hnc{95.8}\;\hnc{\mathbf{96.8}}\;\hnc{\mathbf{96.8}}\;\hnc{\mathbf{97.0}}\;\hnc{\mathbf{97.0}}$ &  $\hnc{92.9}\;\hnc{94.9}\;\hnc{94.9}\;\hnc{95.6}\;\hnc{95.6}\;\hnc{\mathbf{96.4}}\;\hnc{\mathbf{96.4}}$ &  $\hnc{6.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}$ \\
		LSUN-R         &  $\hnc{13.8}\;\hnc{17.8}\;\hnc{12.9}\;\hnc{10.1}\;\hnc{\mathbf{7.3}}\;\hnc{9.3}\;\hnc{\mathbf{7.3}}$ &  $\hnc{97.5}\;\hnc{97.0}\;\hnc{97.6}\;\hnc{98.2}\;\hnc{\mathbf{98.5}}\;\hnc{\mathbf{98.2}}\;\hnc{\mathbf{98.5}}$ &  $\hnc{97.7}\;\hnc{97.4}\;\hnc{97.8}\;\hnc{\mathbf{98.4}}\;\hnc{\mathbf{98.6}}\;\hnc{\mathbf{98.4}}\;\hnc{\mathbf{98.5}}$ &  $\hnc{97.4}\;\hnc{96.6}\;\hnc{97.5}\;\hnc{98.0}\;\hnc{\mathbf{98.4}}\;\hnc{98.0}\;\hnc{\mathbf{98.3}}$ &  $\hnc{6.0}\;\hnc{0.0}\;\hnc{8.0}\;\hnc{0.0}\;\hnc{6.0}\;\hnc{0.0}\;\hnc{7.0}$ \\
		iSUN           &  $\hnc{13.9}\;\hnc{18.3}\;\hnc{13.7}\;\hnc{12.4}\;\hnc{\mathbf{9.0}}\;\hnc{12.2}\;\hnc{9.3}$ &  $\hnc{97.5}\;\hnc{96.9}\;\hnc{97.5}\;\hnc{97.8}\;\hnc{\mathbf{98.2}}\;\hnc{97.8}\;\hnc{\mathbf{98.2}}$ &  $\hnc{97.9}\;\hnc{97.6}\;\hnc{97.8}\;\hnc{\mathbf{98.3}}\;\hnc{\mathbf{98.5}}\;\hnc{\mathbf{98.3}}\;\hnc{\mathbf{98.5}}$ &  $\hnc{97.0}\;\hnc{96.2}\;\hnc{97.1}\;\hnc{97.3}\;\hnc{\mathbf{97.9}}\;\hnc{97.3}\;\hnc{\mathbf{97.9}}$ &  $\hnc{6.0}\;\hnc{0.0}\;\hnc{8.0}\;\hnc{0.0}\;\hnc{6.0}\;\hnc{0.0}\;\hnc{6.0}$ \\
		Gaussiov šum   &  $\hnc{\mathbf{0.0}}\;\hnc{3.2}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}$ &  $\hnc{98.7}\;\hnc{98.4}\;\hnc{99.2}\;\hnc{\mathbf{99.8}}\;\hnc{\mathbf{99.8}}\;\hnc{\mathbf{100.0}}\;\hnc{\mathbf{100.0}}$ &  $\hnc{99.2}\;\hnc{98.9}\;\hnc{99.4}\;\hnc{\mathbf{99.9}}\;\hnc{\mathbf{99.9}}\;\hnc{\mathbf{100.0}}\;\hnc{\mathbf{100.0}}$ &  $\hnc{96.7}\;\hnc{95.8}\;\hnc{97.8}\;\hnc{\mathbf{99.8}}\;\hnc{\mathbf{99.8}}\;\hnc{\mathbf{100.0}}\;\hnc{\mathbf{100.0}}$ &  $\hnc{2.0}\;\hnc{0.0}\;\hnc{2.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}$ \\
		Uniformni šum  &  $\hnc{\mathbf{0.0}}\;\hnc{3.2}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}$ &  $\hnc{98.7}\;\hnc{98.4}\;\hnc{99.2}\;\hnc{\mathbf{99.8}}\;\hnc{\mathbf{99.8}}\;\hnc{\mathbf{100.0}}\;\hnc{\mathbf{100.0}}$ &  $\hnc{99.2}\;\hnc{98.9}\;\hnc{99.4}\;\hnc{\mathbf{99.9}}\;\hnc{\mathbf{99.9}}\;\hnc{\mathbf{100.0}}\;\hnc{\mathbf{100.0}}$ &  $\hnc{96.7}\;\hnc{95.8}\;\hnc{97.8}\;\hnc{\mathbf{99.8}}\;\hnc{\mathbf{99.8}}\;\hnc{\mathbf{100.0}}\;\hnc{\mathbf{100.0}}$ &  $\hnc{2.0}\;\hnc{0.0}\;\hnc{2.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}$ \\
		\bottomrule
	\end{tabular}
	\endgroup
}
		\caption{DN-100-12}
		\label{tab:ood-dn-cifar-poboljsano}
	\end{subtable}
	\vskip\baselineskip
	\begin{subtable}[t]{1\textwidth}
\resizebox{\textwidth}{!}{%
	...
	\begin{tabular}{lrrrrr}
		...
		{} &                                                                                                                           $\mathit{FPR}_{R=0.95}/\%$ &
		$\mathit{AUROC}/\%$ &
		$\mathit{AP}/\%$ &                                                                                                                   $\mathit{AP}_\text{n}/\%$ &
		$\epsilon/10^{-3}$ \\
		\midrule
		%CIFAR-10       &  $\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}\;\hnc{\mathbf{95.0}}$ &  $\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}\;\hnc{\mathbf{50.0}}$ &  $\hnc{50.0}\;\hnc{\mathbf{75.0}}\;\hnc{\mathbf{75.0}}\;\hnc{50.0}\;\hnc{50.0}\;\hnc{\mathbf{75.0}}\;\hnc{\mathbf{75.0}}$ &  $\hnc{50.0}\;\hnc{\mathbf{75.0}}\;\hnc{\mathbf{75.0}}\;\hnc{50.0}\;\hnc{50.0}\;\hnc{\mathbf{75.0}}\;\hnc{\mathbf{75.0}}$ &  $\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}$ \\
		TinyImageNet-C &  $\hnc{31.9}\;\hnc{34.2}\;\hnc{30.8}\;\hnc{21.1}\;\hnc{21.9}\;\hnc{\mathbf{19.5}}\;\hnc{20.2}$ &  $\hnc{93.5}\;\hnc{93.9}\;\hnc{93.3}\;\hnc{95.9}\;\hnc{95.4}\;\hnc{\mathbf{96.2}}\;\hnc{\mathbf{95.9}}$ &  $\hnc{93.6}\;\hnc{94.3}\;\hnc{93.1}\;\hnc{\mathbf{96.1}}\;\hnc{95.6}\;\hnc{\mathbf{96.4}}\;\hnc{96.0}$ &  $\hnc{93.3}\;\hnc{93.2}\;\hnc{93.3}\;\hnc{\mathbf{95.5}}\;\hnc{95.1}\;\hnc{\mathbf{95.8}}\;\hnc{\mathbf{95.7}}$ &  $\hnc{3.0}\;\hnc{0.0}\;\hnc{6.0}\;\hnc{0.0}\;\hnc{2.0}\;\hnc{0.0}\;\hnc{2.0}$ \\
		TinyImageNet-R &  $\hnc{42.3}\;\hnc{42.5}\;\hnc{41.7}\;\hnc{35.0}\;\hnc{36.3}\;\hnc{\mathbf{34.2}}\;\hnc{\mathbf{34.2}}$ &  $\hnc{89.2}\;\hnc{90.8}\;\hnc{89.7}\;\hnc{\mathbf{92.2}}\;\hnc{91.9}\;\hnc{\mathbf{92.3}}\;\hnc{\mathbf{92.3}}$ &  $\hnc{87.3}\;\hnc{90.0}\;\hnc{88.3}\;\hnc{\mathbf{91.5}}\;\hnc{91.1}\;\hnc{\mathbf{91.7}}\;\hnc{\mathbf{91.7}}$ &  $\hnc{89.5}\;\hnc{90.4}\;\hnc{89.9}\;\hnc{\mathbf{91.9}}\;\hnc{91.6}\;\hnc{\mathbf{92.1}}\;\hnc{\mathbf{92.1}}$ &  $\hnc{3.0}\;\hnc{0.0}\;\hnc{3.0}\;\hnc{0.0}\;\hnc{1.0}\;\hnc{0.0}\;\hnc{0.0}$ \\
		LSUN-C         &  $\hnc{16.2}\;\hnc{12.9}\;\hnc{12.9}\;\hnc{\mathbf{11.7}}\;\hnc{\mathbf{11.7}}\;\hnc{\mathbf{11.5}}\;\hnc{\mathbf{11.5}}$ &  $\hnc{96.8}\;\hnc{97.4}\;\hnc{97.4}\;\hnc{\mathbf{97.6}}\;\hnc{\mathbf{97.6}}\;\hnc{\mathbf{97.8}}\;\hnc{\mathbf{97.8}}$ &  $\hnc{96.8}\;\hnc{97.3}\;\hnc{97.3}\;\hnc{\mathbf{97.7}}\;\hnc{\mathbf{97.7}}\;\hnc{\mathbf{97.8}}\;\hnc{\mathbf{97.8}}$ &  $\hnc{96.6}\;\hnc{\mathbf{97.6}}\;\hnc{\mathbf{97.6}}\;\hnc{\mathbf{97.5}}\;\hnc{\mathbf{97.5}}\;\hnc{\mathbf{97.8}}\;\hnc{\mathbf{97.8}}$ &  $\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}$ \\
		LSUN-R         &  $\hnc{22.6}\;\hnc{23.5}\;\hnc{20.1}\;\hnc{17.9}\;\hnc{17.3}\;\hnc{\mathbf{15.1}}\;\hnc{\mathbf{15.1}}$ &  $\hnc{94.6}\;\hnc{95.6}\;\hnc{95.8}\;\hnc{96.6}\;\hnc{96.6}\;\hnc{\mathbf{97.0}}\;\hnc{\mathbf{97.0}}$ &  $\hnc{93.8}\;\hnc{95.9}\;\hnc{95.8}\;\hnc{\mathbf{96.9}}\;\hnc{\mathbf{96.8}}\;\hnc{\mathbf{97.0}}\;\hnc{\mathbf{97.0}}$ &  $\hnc{94.9}\;\hnc{95.2}\;\hnc{95.8}\;\hnc{96.4}\;\hnc{96.4}\;\hnc{\mathbf{96.9}}\;\hnc{\mathbf{96.9}}$ &  $\hnc{6.0}\;\hnc{0.0}\;\hnc{4.0}\;\hnc{0.0}\;\hnc{1.0}\;\hnc{0.0}\;\hnc{0.0}$ \\
		iSUN           &  $\hnc{25.1}\;\hnc{27.1}\;\hnc{22.8}\;\hnc{20.8}\;\hnc{20.2}\;\hnc{\mathbf{18.0}}\;\hnc{\mathbf{17.7}}$ &  $\hnc{94.0}\;\hnc{94.8}\;\hnc{95.0}\;\hnc{\mathbf{96.0}}\;\hnc{\mathbf{96.0}}\;\hnc{\mathbf{96.3}}\;\hnc{\mathbf{96.2}}$ &  $\hnc{93.8}\;\hnc{95.5}\;\hnc{95.5}\;\hnc{\mathbf{96.5}}\;\hnc{\mathbf{96.4}}\;\hnc{\mathbf{96.6}}\;\hnc{\mathbf{96.5}}$ &  $\hnc{93.8}\;\hnc{93.8}\;\hnc{94.6}\;\hnc{95.2}\;\hnc{95.4}\;\hnc{\mathbf{95.7}}\;\hnc{\mathbf{95.7}}$ &  $\hnc{6.0}\;\hnc{0.0}\;\hnc{5.0}\;\hnc{0.0}\;\hnc{1.0}\;\hnc{0.0}\;\hnc{1.0}$ \\
		Gaussiov šum   &  $\hnc{\mathbf{0.0}}\;\hnc{7.4}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}$ &  $\hnc{98.8}\;\hnc{98.0}\;\hnc{98.7}\;\hnc{\mathbf{99.5}}\;\hnc{\mathbf{99.5}}\;\hnc{\mathbf{99.6}}\;\hnc{\mathbf{99.6}}$ &  $\hnc{99.2}\;\hnc{98.7}\;\hnc{99.1}\;\hnc{\mathbf{99.7}}\;\hnc{\mathbf{99.7}}\;\hnc{\mathbf{99.7}}\;\hnc{\mathbf{99.7}}$ &  $\hnc{98.0}\;\hnc{96.7}\;\hnc{97.3}\;\hnc{\mathbf{99.3}}\;\hnc{\mathbf{99.3}}\;\hnc{\mathbf{99.4}}\;\hnc{\mathbf{99.4}}$ &  $\hnc{10.0}\;\hnc{0.0}\;\hnc{2.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}$ \\
		Uniformni šum  &  $\hnc{\mathbf{0.0}}\;\hnc{7.4}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}\;\hnc{\mathbf{0.0}}$ &  $\hnc{98.8}\;\hnc{98.0}\;\hnc{98.7}\;\hnc{\mathbf{99.5}}\;\hnc{\mathbf{99.5}}\;\hnc{\mathbf{99.6}}\;\hnc{\mathbf{99.6}}$ &  $\hnc{99.2}\;\hnc{98.7}\;\hnc{99.1}\;\hnc{\mathbf{99.7}}\;\hnc{\mathbf{99.7}}\;\hnc{\mathbf{99.7}}\;\hnc{\mathbf{99.7}}$ &  $\hnc{98.0}\;\hnc{96.7}\;\hnc{97.3}\;\hnc{\mathbf{99.3}}\;\hnc{\mathbf{99.3}}\;\hnc{\mathbf{99.4}}\;\hnc{\mathbf{99.4}}$ &  $\hnc{10.0}\;\hnc{0.0}\;\hnc{2.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}\;\hnc{0.0}$ \\
		\bottomrule
	\end{tabular}
	...
	\endgroup
		...
		\caption{WRN-28-10}
		\label{tab:ood-wrn-cifar-poboljsano}
	\end{subtable}
	\caption{Razlikovanje izvanrazdiobnih i unutarrazdiobnih primjera kod mreža naučenih na skupu CIFAR-10 uz poboljšanja. Za svaku evaluacijsku mjeru $5$ stupaca redom predstavlja klasifikaciju: (1) po maksimalnom logitu uz FGSM, (2) logističkom regresijom po maksimalnom logitu i zbroju logita, (3) isto uz FGSM, (4) po normalizirianom maksimalnom logitu, (5) isto uz FGSM, (6) logističkom regresijom po normaliziranom maksimalnom logitu i zbroju logita i (7) isto uz FGSM. Podebljani su najbolji rezultati (više njih ako se razlikuju od najboljeg za manje od $0.003$) za svaki par izvanrazdiobnog skupa i evaluacijske mjere.}
\end{table}
...
Jedan pristup se temelji na aproksimaciji bayesovskih neuronskih mreža kod kojih se provodi bayesovska procjena parametara, teorijski opravdan način rješavanja problema procjene nesigurnosti. Oni omogućuju procjenu nesigurnosti i razlikovanje je li uzrok nesigurnosti višeznačnost podatka ili nesigurnost u parametre modela, ali, i uz jako jednostavnu varijacijsku razdiobu kojom se aproksimira aposteriorna razdioba parametara, zahtijevaju puno više računanja u odnosu na uobičajene neuronske mreže. \citet{Gal:2016:BCNNBAVI} su učenje mreže s \textit{dropoutom} interpretirali kao varijacijsko zaključivanje. Za ovaj rad je ispitan pristup za procjenu i razlikovanje nesigurnosti kod semantičke segmentacije koji se temelji na \textit{Monte Carlo dropoutu} i idejama iz \citet{Kendall:2017:WUNBDLCV,Smith:2018:UMUAED}. Rezultati eksperimenata pokazuju manju uspješnost na ispitivanom modelu u odnosu na rezultate u \citet{Kendall:2017:WUNBDLCV}. 

Druga skupina razmatranih pristupa su pristupi za prepoznavanje primjera koji su izvan razdiobe skupa za učenje \citet{Guo:2017:CMNN,Hendrycks:2016:BDMOODE,Liang:2017:PDOODENN} na temelju maksimalnog izlaza softmaksa ili logita kod unaprijed naučenih modela. Postupci opisani u \citet{Hendrycks:2016:BDMOODE,Liang:2017:PDOODENN}, koji za klasifikaciju koriste maksimalni izlaz softmaksa, uspoređeni su s analognim postupcima koji umjesto maksimalnog izlaza softmaksa koriste maksimalni logit. Rezultati eksperimenta na korištenim modelima i skupovima pokazuju da postupak klasifikacije na temelju vrijednosti maksimalnog logita uz pomak ulaznog primjera u smjeru predznaka gradijenta kao u \citet{Liang:2017:PDOODENN} daje skoro jednake rezultate kao postupak klasifikacije maksimalnog izlaza softmaksa s pomakom predložen u \citet{Liang:2017:PDOODENN}. Predložena su i ispitana neka poboljšanja prepoznavanja izvanrazdiobnih primjera na temelju logita. Jedno je normalizacija logita po klasama. Drugo je klasifikacija većeg broja značajki izvedenih iz vektora logita plitkim klasifikatorom, gdje se kao značajke za klasifikaciju koriste maksimalni logit i zbroj logita.

Za budući rad bi se mogli dublje empirijski i teorijski istražiti obrađeni pristupi. Mogli bi se bolje interpretirati i rezultati procjene epistemičke i aleatorne nesigurnosti. Bilo bi dobro i objasniti zašto kod prepoznavanje izvanrazdiobnih primjera pomaže normalizacija logita. Rezultati pokazuju da se postupci za prepoznavanje izvanrazdiobnih primjera na već naučenoj mreži vjerojatno mogu još poboljšati.


%Prepoznavanje izvanrazdiobnih primjera bi se vjerojatno moglo poboljšati uzimanjem u obzir većeg broja značajki za klasifikaciju
...
\begin{sazetak}
Ovaj rad razmatra problem procjene nesigurnosti predikcija kod dubokih modela. Obrađena su dva pristupa procjene nesigurnosti. Cilj jednog pristupa je postići pouzdanu procjenu nesigurnosti i prepoznati je li uzrok nesigurnosti višeznačnost podataka ili nesigurnost u parametre aproksimacijom bayesovskih neuronskih mreža. Cilj druge skupine pristupa je poboljšati procjenu pouzdanosti predikcije bez mijenjanja već naučene mreže i prepoznati je li ulazni primjer izvan razdiobe skupa za učenje ili je krivo klasificiran na temelju izlaza softmaksa ili logita. Ispitani su neki postupci na modelima za semantičku segmentaciju i klasifikaciju slika. Pokazano je da se postupak za prepoznavanje primjera koji su izvan razdiobe skupa za učenje može poboljšati uzimanjem u obzir većeg broja značajki izvedenih iz vektora logita.

\kljucnerijeci{duboko učenje, strojno učenje, nesigurnost, statističko zaključivanje, varijacijsko zaključivanje, bayesovske neuronske mreže, prepoznavanje izvanrazdiobnih primjera, računalni vid}
\end{sazetak}

\newpage
\engtitle{Supervised approaches to estimation of predictive uncertainty in deep models}
\begin{abstract}
This thesis considers the problem of predictive uncertainty estimation in deep models. Two approaches for uncertainty estimation are considered. The goal of one of them is to achieve reliable uncertainty estimation and to distinguish whether the cause of uncertainty is ambiguity of data or parameter uncertainty caused by lack of data by approximating Bayesian neural networks. The goal of the other approach is to improve predictive uncertainty estimation without changing a trained neural network and to detect out-of-distribution or misclassified examples based on the softmax output or logits. It is shown that out-of-distribution example detection can be improved by taking more features derived from the logit vector into account.
