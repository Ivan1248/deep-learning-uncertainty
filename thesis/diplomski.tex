...
Možemo razmatrati više slučajnih varijable zajedno (združenu slučajnu varijablu) i njihovu združenu razdiobu $p(\rvar x,\rvar y)$. Događaji onda imaju oblik $\cbr{R(\rvar x,\rvar y)}$. Elementarni događaj onda ima oblik $\cbr{\rvar x=x, \rvar y=y}$. Dalje će $x$ označavati $\cbr{\rvar x=x}$ kada je jasno po slovu o kojoj se slučajnoj varijabli radi.
...
\end{align}
$P(x\mid y)$ označava \emph{uvjetnu vjerojatnost} događaja $\cbr{\rvar x=x}$ ako je poznato da se ostvario događaj $\cbr{\rvar y=y}$. Ako su $\rvar x$ i $\rvar y$ nezavisne, onda vrijedi $P(x,y) = P(x)P(y)$. \emph{Marginalna vjerojatnost} slučajne varijable $\rvar x$ je $P(x) = P(x,y\in\set Y)$, gdje je $\set Y$ skup mogućih vrijednosti slučajne varijable $\rvar y$. Izraženo gustoćom vjerojatnosti (\emph{pravilo zbroja}):
\begin{align}
...
\begin{align}
P(A\mid B) = \frac{P(B\mid A)P(A)}{P(B)} \text{,}
\end{align}
što možemo i ovako zapisati:
\begin{align}
p(x\mid y) = \frac{p(y\mid x)p(x)}{\int p(y\mid x)p(x)\dif x} \text{,}
\end{align}
gdje se nazivnih integrira po svim vrijednostima.
...
\begin{align}
\hat{\theta}=g(x_1,\dots,x_N) \text{.}
\end{align}
Ako kao parametre takve funkcije uzmemo \emph{uzorak}, tj. skup slučajnih varijabli $\rset D=\del{\rvar x_1,\dots,\rvar x_N}$ koje su nezavisne i imaju istu razdiobu kao $\rvar x$ dobivamo slučajnu varijablu
\begin{align}
\hat{\rvar\theta}=g(\rset D) \text{.}
\end{align}
...
Može se pokazati da je srednja kvadratna pogreška procjenitelja $\E\sbr{\del{\hat{\rvar\theta}-\theta}^2}$ jednaka zbroju varijance i kvadrata pristranosti procjenitelja \citep{Snajder:2014:SU}.
