...
\subsubsection{Skupovi podataka}
...
\begin{solidenumerate}
	...
	\item Cityscapes \citep{Cordts:2016:Cityscapes} -- skup slika dimenzija $2048\times1024$ iz vožnje u gradovima s oznakama za $19$ klasa. Skup za učenje ima $2975$ slika, skup za validaciju $500$, a skup za testiranje $1525$. Nisu dostupne ciljne oznake skupa za testiranje. Za učenje i ispitivanje su korištene slike umanjene uz odsječen donji dio, koji uglavnom činu neoznačena hauba, tako da budu dimenzije $1024\times448$.
	\item WildDash\footnote{\url{http://www.wilddash.cc/}} -- skup iz vožnje s različitim vremenskim uvjetima, osvjetljenjima, okolinama, vozilima, rijetkim pojavama i s različitim parametrima kamere. Slike su dimenzija $1920\times1080$. Oznake su iste kao kod Cityscapesa. Za eksperimente je korišten podskup $\text{WildDash}_\text{bench}$ koji nema oznake i sadrži posebno teške slike od kojih su neke izobličene i neke nisu slike prometa i ne bi se trebale moći označiti i slično.
\end{solidenumerate}

\subsubsection{Modeli}

Umjesto modela koji su koristili \citet{Kendall:2017:WUNBDLCV}, korištena je mreža LadderDenseNet-121 \citep{Kreso:2017:LSDFSSLNI}, za koju je potrebno manje memorije. Ona se temelji na mreži DenseNetBC-121 \citep{Huang:2016:DCCN} i prilagođena je za semantičku segmentaciju. Korištena je vlastita implementacija te mreže koja ne postiže jednako dobru performansu kao originalna iako nije poznata značajna razlika u inicijalizaciji i učenju. Ta mreža će se označavati s \textit{LadderDensenet-121-V}. Na validacijskom skupu Cityscapesa LadderDensenet-121-V bez \textit{dropouta} postiže srednji omjer presjeka i unije $\mIoU=0.6721\pm0.007$ i točnost piksela $A=0.9459\pm0.0006$ (sredina i standardna devijacija skupa od $5$ mjerenja), što je značajno manje od $\mIoU=0.7282$ i $A=0.9506$ kod originalne implementacije. Mreža učena na CamVidu uz isti postupak učenja postiže $\mIoU=0.6774\pm0.0031$ i $A=0.9176\pm0.0025$ na skupu za testiranje CamVida. Pokazalo se da dodavanje \textit{dropouta} s vjerojatnošću isključivanja $1-p=0.2$, kao kod \citep{Huang:2016:DCCN}, loše utječe na performansu. Čini se da ni vjerojatnost isključivanja $0.1$ nema značajno pozitivan utjecaj na rezultat u odnosu na mrežu bez \textit{dropouta}, pogotovo za Cityscapes, koji je veći skup od CamVida. 
...
U tablici~\ref{tab:evaluacija-camvid} su prikazani rezultati evaluacije različitih modela učenih na CamVidu (osim podskupa za testiranje). Može se vidjeti da \textit{MC-dropout} ima pozitivan učinak na evaluaciju u odnosu na obični \textit{dropout}.
...
\begin{table}
	...
	\begin{tabular}{llrrrr}
		...
		\midrule
		$\text{CamVid}_\text{trainval}$ & $\text{CamVid}_\text{test}$ & $0.663$ & $0.207$ & $0.025$ & $0.121$ \\
		$\text{CamVid}_{\text{trainval},1/2}$ & $\text{CamVid}_\text{test}$ & $0.615$ & $0.272$ & $0.026$ & $0.097$ \\
		$\text{CamVid}_{\text{trainval},1/4}$ & $\text{CamVid}_\text{test}$ & $0.546$ & $0.391$ & $0.029$ & $0.073$ \\
		$\text{CamVid}_{\text{trainval},1/8}$ & $\text{CamVid}_\text{test}$ & $0.460$ & $0.538$ & $0.034$ & $0.063$ \\
		\midrule
		$\text{CamVid}_\text{trainval}$     & $\text{CamVid}_\text{val}$ & $0.827$ & $0.118$ & $0.011$ & $0.093$ \\
		\midrule
		$\text{CamVid}_\text{trainval}$  & $\text{Cityscapes}_\text{test}$ &      -  & $0.383$ & $0.060$ & $0.156$ \\
		$\text{CamVid}_\text{trainval}$  & 	$\text{WildDash}_\text{bench}$	 &      -  & $0.501$ & $0.075$ & $0.149$ \\
		\midrule
		$\text{Cityscapes}_\text{train}$ & $\text{Cityscapes}_\text{val}$  & $0.644$ & $0.187$ & $0.024$ & $0.126$ \\
		$\text{Cityscapes}_\text{train}$ & $\text{Cityscapes}_\text{test}$ & -       & $0.162$ & $0.020$ & $0.122$ \\
		$\text{Cityscapes}_\text{train}$ & $\text{WildDash}_\text{bench}$  & - & $0.600$ & $0.153$ & $0.254$ \\
		\bottomrule
	\end{tabular}
	\caption{Procjene aleatorne i epistemičke nesigurnosti za različite parove skupa za učenje i skupa za ispitivanje i njihovi omjeri. U indeksima su imena podskupova. $1/n$ u indeksu označava da se uzima slučajan podskup s $1/n$ od svih primjera.}
...
\begin{solidenumerate}
	...
	\item procjenu epistemičke nesigurnosti procjenom međusobne informacije prema izrazu~\eqref{eq:mc-dropout-epistemicka nesigurnost} pomnožena s $5$ za svaki piksel.
\end{solidenumerate}
Kod slika koje predstavljaju nesigurnost boje od crne do bijele predstavljaju vrijednosti iz intervala $\intcc{0,\ln C}$, gdje je $C=19$ broj klasa. Na slici~\ref{fig:boje-klasa-cityscapesa} se vidi koja boja predstavlja koju klasu kod semantičke segmentacije. Mreža je naučena na skupu za učenje Cityscapesa s \textit{dropoutom}.
...
\begin{figure}
	...
	\begin{minipage}[b]{1.9cm}\offinterlineskip
		\begin{tabu} {@{}X[1,c,m]@{}@{}X[-1,c,m]@{}}
			...
			\makecell[l]{epistemička\\ nesigurnost\\$\cdot5$ } &\vspace{1.6cm} \\
		\end{tabu}
	\end{minipage}
...
\end{minipage}
	...
	\caption{Predikcije i procjene nesigurnosti na odabranim slikama iz skupa WildDash na kojima se vide veća razlike između procjena epistemičke i aleatorne nesiguernosti.}
...
\item maksimalne vrijednosti softmaksa uz $T=1000$ %(ili logita s pomakom ulaza uz $T\gg 1$)
...
\begin{table} \centering
	...
	\caption{Prepoznavanje krivo klasificiranih primjera kod mreža naučenih na skupu CIFAR-10. Stupci pod evaluacijskim mjerama imaju isto značenja kao u prethodnim tablicama. Podebljani su najbolji rezultati (više njih ako se razlikuju od najboljeg za manje od $0.003$) za svaku evaluacijsku mjeru.}
...
Za budući rad bi se mogli dublje empirijski i teorijski istražiti obrađeni pristupi. Postupci za prepoznavanje izvanrazdiobnih primjera bi se možda mogli poboljšati uzimanjem u obzir klase kojoj odgovara maksimalni logit ili korištenjem većeg broja značajki.
