...
\end{align}
gdje je $\set X$ prostor elementarnih događaja slučajne varijable $\rvar x$, a $\dirac$ Diracova delta, poopćena funkcija za koju vrijedi $\dirac(x)=0$ za $x\neq0$ i $\int_x\dirac(x)\dif x=1$. Diracova delta se može promatrati kao limes funkcije gustoće Gaussove razdiobe:
\begin{align*}
...
Ovaj odjeljak se uglavnom temelji na \cite{Grubisic:2018:IPM}.
...
\widetilde{\vec x}_i = \Pi_{B_\epsilon(\vec x)} (\widetilde{\vec x}_{i-1} + \alpha\sgn\nabla_{\widetilde{\vec x}_{i-1}} L(y,h(\widetilde{\vec x}_{i-1}))) \text{.}
\end{equation}
...
R_\text{A}(\vec\theta;d,\epsilon) = \E_{(\vec x,y)\sim\mathcal{D}}\left[
...
\section{Aproksimacija bayesovske neuronske mreže s dropoutom}
