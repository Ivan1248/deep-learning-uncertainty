...
#   mozgalo rn 50 64
#   mozgalo rn 18 64 /home/igrubisic/projects/dl-uncertainty/data/nets/mozgalo-trainval/rn-18-64-e12/2018-05-21-1208/Model
...
ds_id_to_ds = {
    ds_id: data_utils.get_cached_dataset_with_normalized_inputs(
        ds_id, trainval_test=True)[1].subset(np.arange(2000))
    for ds_id in ['cifar', 'tinyimagenet', 'isun', 'mozgalo', 'cityscapes']
}
...
for ds_id, ds in list(ds_id_to_ds.items()):
    if ds_id == args.ds:
        continue
    if not (args.ds == 'cifar' and ds_id in ['isun', 'mozgalo']):
...
ds_id_to_logits_sum_ds = {
    k: v.map(lambda xy: (np.sum(xy[0]), xy[1]))
    for k, v in ds_id_to_logits_ds.items()
}

max_logits_ds = ds_id_to_max_logits_ds[args.ds]
max_logits_range = (np.min(max_logits_ds), np.max(max_logits_ds))
